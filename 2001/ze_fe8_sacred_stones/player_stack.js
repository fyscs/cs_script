...
const SCRIPT_PREFIX = 'player_stack';
// ===================================\\
// 工具函数
// ===================================\\
// 获取玩家的 stack 值，若不存在则初始化为 0
function getPlayerStack(player) {
    const state = getEntityState(player);
...
// 增加玩家的 stack（+1）
function incrementPlayerStack(player) {
    const state = getEntityState(player);
    if (state) {
        if (!('stack' in state))
            state.stack = 0;
...
function applyDamage(player, damage) {
    ...
    if (hp - damage <= 0) {
        EntFireTarget(player, 'Kill');
        return;
    }
    EntFireTarget(player, 'SetHealth', (hp - damage).toString());
}
function addToList(inputData) {
    const { activator } = inputData;
    if (activator && activator.IsValid() && getClassname(activator) === 'player' && getTeam(activator) === 3) {
        printl(`[PlayerStack] addToList: added ${activator.GetEntityName()}`);
    }
}
function addActivatorStack(inputData) {
    const { activator } = inputData;
    if (activator && activator.IsValid() && getClassname(activator) === 'player' && getTeam(activator) === 3) {
        // 初始化 stack
        const state = getEntityState(activator);
        if (state && !('stack' in state)) {
            state.stack = 0;
        }
        printl(`[PlayerStack] addActivatorStack: initialized stack for ${activator.GetEntityName()}`);
    }
}
// 带堆叠增加（根据 stack 值乘伤害）
function checkActivatorStack(inputData, dmg) {
    const { activator } = inputData;
    if (!activator || !activator.IsValid() || getClassname(activator) !== 'player' || getTeam(activator) !== 3)
        return;
    if (activator.GetHealth() <= 0)
        return;
    const stack = getPlayerStack(activator);
    let damage = dmg;
    if (stack === 2) {
        damage = dmg * 3;
    }
    ...
    // stack 0 和 1 使用原始伤害
    applyDamage(activator, damage);
    // 除了 stack=3 之外，都增加 stack
    if (stack !== 3) {
        incrementPlayerStack(activator);
    }
    printl(`[PlayerStack] checkActivatorStack: dmg=${damage}, stack=${stack} -> ${stack !== 3 ? stack + 1 : stack}`);
}
// 不带堆叠增加
function checkActivatorStackNoAdd(inputData, dmg) {
    const { activator } = inputData;
    if (!activator || !activator.IsValid() || getClassname(activator) !== 'player' || getTeam(activator) !== 3)
        return;
    if (activator.GetHealth() <= 0)
    ...
    applyDamage(activator, damage);
    printl(`[PlayerStack] checkActivatorStackNoAdd: dmg=${damage}, stack=${stack}`);
}
// 传染雾：伤害目标，并对周围随机一个 CT 造成少量伤害
function checkContagiousMist(inputData, dmg) {
    const { activator } = inputData;
    if (!activator || !activator.IsValid() || getClassname(activator) !== 'player' || getTeam(activator) !== 3)
        return;
    if (activator.GetHealth() <= 0)
        return;
    // 对激活者造成完整伤害
    applyDamage(activator, dmg);
    // 寻找附近 512 单位内的另一个 CT（不包含自己）
    const origin = getOrigin(activator);
    const players = Instance.FindEntitiesByClass('player');
    let nearby = null;
    for (const p of players) {
        if (!p.IsValid() || p === activator)
            continue;
        if (getTeam(p) === 3 && p.GetHealth() > 0) {
            const dist = distance(origin, getOrigin(p));
            if (dist <= 512) {
                nearby = p;
                break;
            }
    ...
    if (nearby) {
        const cmg = Math.floor((1 / 3) * dmg); // 原脚本为 (1/3)*dmg，可能为整数
        applyDamage(nearby, cmg);
        printl(`[PlayerStack] checkContagiousMist: also damaged nearby ${nearby.GetEntityName()} for ${cmg}`);
    }
}
// 检查玩家死亡（但原脚本是空的，保留）
function checkDead(inputData) {
    const { activator } = inputData;
    if (activator && activator.IsValid() && activator.GetHealth() <= 0) ;
}
// ===================================\\
// 输入别名注册
// ===================================\\
const EXTERNAL_INPUT_ALIASES = [
    input('AddToList', 'AddToList()', addToList, 'vmf', 'Add activator to candidates list'),
    input('AddActivatorStack', 'AddActivatorStack()', addActivatorStack, 'vmf', 'Initialize stack for activator'),
    input('CheckActivatorStack_1', 'CheckActivatorStack(1)', (inputData) => checkActivatorStack(inputData, 1), 'vmf', 'Apply stack damage with dmg=1'),
    input('CheckActivatorStack_5', 'CheckActivatorStack(5)', (inputData) => checkActivatorStack(inputData, 5), 'vmf', 'Apply stack damage with dmg=5'),
    input('CheckActivatorStack_10', 'CheckActivatorStack(10)', (inputData) => checkActivatorStack(inputData, 10), 'vmf', 'Apply stack damage with dmg=10'),
    input('CheckActivatorStackNoAdd_1', 'CheckActivatorStackNoAdd(1)', (inputData) => checkActivatorStackNoAdd(inputData, 1), 'vmf', 'Apply stack damage no add with dmg=1'),
    input('CheckActivatorStackNoAdd_5', 'CheckActivatorStackNoAdd(5)', (inputData) => checkActivatorStackNoAdd(inputData, 5), 'vmf', 'Apply stack damage no add with dmg=5'),
    input('CheckActivatorStackNoAdd_10', 'CheckActivatorStackNoAdd(10)', (inputData) => checkActivatorStackNoAdd(inputData, 10), 'vmf', 'Apply stack damage no add with dmg=10'),
    input('CheckContagiousMist_1', 'CheckContagiousMist(1)', (inputData) => checkContagiousMist(inputData, 1), 'vmf', 'Contagious mist with dmg=1'),
    input('CheckContagiousMist_5', 'CheckContagiousMist(5)', (inputData) => checkContagiousMist(inputData, 5), 'vmf', 'Contagious mist with dmg=5'),
    input('CheckContagiousMist_10', 'CheckContagiousMist(10)', (inputData) => checkContagiousMist(inputData, 10), 'vmf', 'Contagious mist with dmg=10'),
    input('CheckDead', 'CheckDead()', checkDead, 'vmf', 'Check if activator is dead'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[player_stack] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
