...
const SCRIPT_PREFIX = 'formortiis_demonic_light';
// ===================================\\
// 状态变量
// ===================================\\
let formortiis = null;
...
// 核心函数
// ===================================\\
function prepare(inputData) {
    const { caller } = inputData;
    if (!caller || !caller.IsValid()) {
        printl('[DemonicLight] prepare: caller invalid');
        return;
    }
    formortiis = Instance.FindEntityByName('formortiis_hitbox_head');
    if (formortiis && formortiis.IsValid()) {
        printl('[DemonicLight] formortiis found');
    }
    else {
        printl('[DemonicLight] formortiis not found');
    }
}
function getLightTarget() {
    if (!formortiis || !formortiis.IsValid()) {
        printl('[DemonicLight] getLightTarget: formortiis invalid');
        return '0 0 0';
    }
    const origin = getOrigin(formortiis);
    const candidates = [];
    ...
    if (candidates.length === 0) {
        printl('[DemonicLight] getLightTarget: no candidates found');
        return '0 0 0';
    }
    const target = candidates[randomInt(0, candidates.length - 1)];
    const pos = getOrigin(target);
    const result = `${pos.x} ${pos.y} ${pos.z}`;
    printl(`[DemonicLight] getLightTarget: ${result}`);
    return result;
}
function setDemonicLight(inputData) {
    const { caller } = inputData;
    if (!caller || !caller.IsValid()) {
        printl('[DemonicLight] setDemonicLight: caller invalid');
        return;
    }
    const targetPos = getLightTarget();
    EntFire('demonic_light_spawner', 'KeyValues', `origin ${targetPos}`);
    EntFire('demonic_light_spawner', 'ForceSpawn', '', 0.1);
    printl('[DemonicLight] setDemonicLight: spawned light');
}
// ===================================\\
// 输入别名注册
// ===================================\\
const EXTERNAL_INPUT_ALIASES = [
    input('Prepare', 'Prepare()', prepare, 'vmf', 'Find formortiis_hitbox_head entity'),
    input('SetDemonicLight', 'SetDemonicLight()', setDemonicLight, 'vmf', 'Spawn demonic light at random CT position'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[formortiis_demonic_light] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
