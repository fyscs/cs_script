...
const SCRIPT_PREFIX = 'rex_barrier';
// ===================================\\
// 核心函数
// ===================================\\
function onUse(inputData) {
    const { activator, caller } = inputData;
    printl(`[RexBarrier] onUse called`);
    // 必须要有 caller 来获取位置
    if (!caller || !caller.IsValid()) {
        printl('[RexBarrier] onUse: caller invalid');
        return;
    }
    const origin = getOrigin(caller);
    printl(`[RexBarrier] caller origin: ${origin.x}, ${origin.y}, ${origin.z}`);
    const candidates = [];
    // 使用 GetAllPlayerControllers 获取所有玩家控制器
    const allControllers = Instance.GetAllPlayerControllers();
    for (const controller of allControllers) {
        ...
        if (!player || !player.IsValid())
            continue;
        // 检查是否为 CT 阵营 (team 3) 且存活
        if (player.GetTeamNumber() !== 3 || player.GetHealth() <= 0)
            continue;
        // 检查距离
        const dist = distance(origin, getOrigin(player));
        if (dist <= 512) {
            candidates.push(player);
        }
    }
    const count = candidates.length;
    printl(`[RexBarrier] found ${count} CT players within 512 units`);
    if (count <= 3) {
        // 激活失败
        EntFire('server', 'Command', 'say [RexBarrier]: RexBarrier failed to activate');
        EntFire('rexwind_btn', 'Lock');
    }
    else {
        // 成功
        EntFire('server', 'Command', 'say [RexBarrier]: Successful activation');
        EntFire('rexwind_maker', 'ForceSpawn', '', 2.0);
        EntFire('rexwind_btn', 'Lock');
    }
}
// ===================================\\
...
const EXTERNAL_INPUT_ALIASES = [
    input('OnUse', 'OnUse()', onUse, 'vmf', 'Activate RexBarrier'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[rex_barrier] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
