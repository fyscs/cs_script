...
const SCRIPT_PREFIX = 'weakspot_script';
// ===================================\\
// 状态变量
// ===================================\\
let current_weakspot = 1;
// ===================================\\
// 核心函数
// ===================================\\
function changeWeakSpot() {
    const x = randomInt(0, 1);
    switch (current_weakspot) {
        case 1:
            if (x === 0) {
                EntFire('formortiis_weak_train', 'StartForward', '', 1.00);
                current_weakspot = 2;
            }
            else {
                EntFire('weak_path_a1', 'ToggleAlternatePath');
                EntFire('formortiis_weak_train', 'StartForward', '', 1.00);
                current_weakspot = 3;
            }
            break;
        case 2:
            if (x === 0) {
                ...
                EntFire('weak_path_a2', 'ToggleAlternatePath');
                EntFire('formortiis_weak_train', 'StartForward', '', 1.00);
                current_weakspot = 1;
            }
            break;
        case 3:
            if (x === 0) {
                ...
                EntFire('weak_path_a3', 'ToggleAlternatePath');
                EntFire('formortiis_weak_train', 'StartForward', '', 1.00);
                current_weakspot = 2;
            }
            break;
...
const EXTERNAL_INPUT_ALIASES = [
    input('ChangeWeakSpot', 'ChangeWeakSpot()', changeWeakSpot, 'vmf', 'Randomly switch weak spot and trigger path changes'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[weakspot_script] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
