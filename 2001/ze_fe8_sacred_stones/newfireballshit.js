...
const SCRIPT_PREFIX = 'newfireballshit';
// ===================================\\
// 配置常量
// ===================================\\
const RANDOM_GRAV = 0.3;
// ===================================\\
// 状态变量
// ===================================\\
let temp_selector = '';
...
// ===================================\\
function chooser() {
    const luckydrawer = randomFloat(0.0, 1.0);
    if (luckydrawer <= RANDOM_GRAV) {
        temp_selector = 'gravity_template';
        printl('[FireballShit] chooser: gravity');
    }
    else {
        temp_selector = 'fireball_template';
        printl('[FireballShit] chooser: fireball');
    }
}
function getTarget(inputData) {
    const { caller } = inputData;
    if (!caller || !caller.IsValid()) {
        printl('[FireballShit] getTarget: caller invalid');
        return '0 0 0';
    }
    ...
    for (const p of players) {
        ...
        if (dist <= 8000) {
            const pos = getOrigin(p);
            return `${pos.x} ${pos.y} ${pos.z}`;
        }
    }
    printl('[FireballShit] getTarget: no CT found within 8000 units');
    return '0 0 0';
}
function setRandomAtkOrigin(inputData) {
    const { caller } = inputData;
    if (!caller || !caller.IsValid()) {
        printl('[FireballShit] setRandomAtkOrigin: caller invalid');
        return;
    }
    if (temp_selector === '') {
        printl('[FireballShit] setRandomAtkOrigin: temp_selector not set, calling chooser()');
        chooser();
    }
    const targetPos = getTarget(inputData);
    EntFire('level4_fireball_maker', 'KeyValues', `origin ${targetPos}`);
    EntFire('level4_fireball_maker', 'KeyValues', `EntityTemplate ${temp_selector}`);
    EntFire('level4_fireball_maker', 'ForceSpawn', '', 0.1);
    printl(`[FireballShit] setRandomAtkOrigin: spawned ${temp_selector} at ${targetPos}`);
}
// ===================================\\
// 输入别名注册
// ===================================\\
const EXTERNAL_INPUT_ALIASES = [
    input('Chooser', 'Chooser()', chooser, 'vmf', 'Randomly select gravity or fireball template'),
    input('SetRandomAtkOrigin', 'SetRandomAtkOrigin()', setRandomAtkOrigin, 'vmf', 'Spawn selected template at random CT position'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[newfireballshit] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
