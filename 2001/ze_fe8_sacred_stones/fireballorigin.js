...
const SCRIPT_PREFIX = 'fireballorigin';
// ===================================\\
// 状态变量
// ===================================\\
let dragon = null;
...
function prepare(inputData) {
    ...
    if (!caller || !caller.IsValid()) {
        printl('[FireballOrigin] prepare: caller invalid');
        return;
    }
    dragon = Instance.FindEntityByName('level4_boss_hitbox');
    if (dragon && dragon.IsValid()) {
        printl('[FireballOrigin] dragon found');
    }
    else {
        printl('[FireballOrigin] dragon not found');
    }
}
function getFireballTarget() {
    if (!dragon || !dragon.IsValid()) {
        printl('[FireballOrigin] getFireballTarget: dragon invalid');
        return '0 0 0';
    }
    const origin = getOrigin(dragon);
    ...
    for (const p of players) {
        ...
        if (dist <= 5000) {
            ...
            printl(`[FireballOrigin] getFireballTarget: ${result}`);
            return result;
        }
    }
    printl('[FireballOrigin] getFireballTarget: no CT found within 5000 units');
    return '0 0 0';
}
function setFireballOrigin(inputData) {
    const { caller } = inputData;
    if (!caller || !caller.IsValid()) {
        printl('[FireballOrigin] setFireballOrigin: caller invalid');
        return;
    }
    const targetPos = getFireballTarget();
    EntFire('level4_fireball_maker', 'KeyValues', `origin ${targetPos}`);
    EntFire('level4_fireball_maker', 'ForceSpawn', '', 0.1);
    printl('[FireballOrigin] setFireballOrigin: spawned fireball');
}
// ===================================\\
// 输入别名注册
// ===================================\\
const EXTERNAL_INPUT_ALIASES = [
    input('Prepare', 'Prepare()', prepare, 'vmf', 'Find level4_boss_hitbox entity'),
    input('SetFireballOrigin', 'SetFireballOrigin()', setFireballOrigin, 'vmf', 'Spawn fireball at random CT position'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[fireballorigin] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
