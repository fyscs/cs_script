...
const SCRIPT_PREFIX = 'skin_item_animation';
// ===================================\\
// 配置常量
// ===================================\\
const TICKRATE = 0.17;
...
let model = null;
let Ticking = false;
let prev_location = { x: 0, y: 0, z: 0 };
let current_location = { x: 0, y: 0, z: 0 };
...
function Start(inputData) {
    printl('[SkinItemAnim] Start called');
    if (!item_owner || !item_owner.IsValid()) {
        printl('[SkinItemAnim] Start: item_owner is null or invalid');
        return;
    }
    Ticking = true;
    prev_location = getOrigin(item_owner);
    current_location = getOrigin(item_owner);
    if (!loopScheduled) {
        loopScheduled = true;
        velocAnimation();
    }
...
function Pause() {
    printl('[SkinItemAnim] Pause called');
    Ticking = false;
    loopScheduled = false;
}
function Tick(value) {
...
function velocAnimation() {
    if (!Ticking) {
        loopScheduled = false;
        return;
    }
    scheduleScript(SCRIPT_PREFIX, () => velocAnimation(), TICKRATE, null, null);
    if (!item_owner || !item_owner.IsValid()) {
        printl('[SkinItemAnim] velocAnimation: item_owner invalid');
        return;
    }
    current_location = getOrigin(item_owner);
    const velocity = getDistance(prev_location, current_location) / TICKRATE;
    prev_location = current_location;
    let playbackValue = (Math.floor(velocity) / 3) * 0.01;
    if (playbackValue > 1.0)
        playbackValue = 1.0;
    if (Math.floor(velocity) <= 0) {
        if (model && model.IsValid()) {
            EntFireTarget(model, 'SetAnimationLooping', 'Idle');
            EntFireTarget(model, 'SetIdleAnimationLooping', 'Idle');
            EntFireTarget(model, 'SetPlaybackRate', '1');
        }
    }
    else {
        if (model && model.IsValid()) {
            EntFireTarget(model, 'SetAnimationLooping', 'Walk');
            EntFireTarget(model, 'SetIdleAnimationLooping', 'Walk');
            EntFireTarget(model, 'SetPlaybackRate', `${playbackValue}`);
        }
    }
}
function onDamaged(inputData) {
    const { activator } = inputData;
    if (!activator || !activator.IsValid()) {
        printl('[SkinItemAnim] onDamaged: activator invalid');
        return;
    }
    if (!item_owner || !item_owner.IsValid()) {
        printl('[SkinItemAnim] onDamaged: item_owner invalid');
        return;
    }
    const fwd = getForwardVector(activator);
    const knockX = Math.floor(KNOCKBACK * fwd.x);
    const knockY = Math.floor(KNOCKBACK * fwd.y);
    const modifierString = `basevelocity ${knockX} ${knockY} 0`;
    EntFireTarget(item_owner, 'KeyValues', modifierString);
}
function setHandler(inputData, value) {
    const { activator } = inputData;
    if (!activator || !activator.IsValid()) {
        printl('[SkinItemAnim] setHandler: activator invalid');
        return;
    }
    item_owner = activator;
    switch (value) {
        case 1:
            model = Instance.FindEntityByName('less_demon_mdl');
            break;
        case 2:
            model = Instance.FindEntityByName('skeleton_mdl');
            break;
        case 3:
            model = Instance.FindEntityByName('nightmare_mdl');
            break;
        default:
            printl(`[SkinItemAnim] setHandler: unknown value ${value}`);
            return;
    }
    if (model && model.IsValid()) {
        printl(`[SkinItemAnim] setHandler: model set to ${model.GetEntityName()}`);
    }
    else {
        printl(`[SkinItemAnim] setHandler: model not found for value ${value}`);
    }
}
function setKnockback(inputData, value) {
    KNOCKBACK = value;
    printl(`[SkinItemAnim] setKnockback: KNOCKBACK = ${KNOCKBACK}`);
}
// ===================================\\
// 输入别名注册
// ===================================\\
const EXTERNAL_INPUT_ALIASES = [
    input('Start', 'Start()', Start, 'vmf', 'Start the animation loop'),
    input('Pause', 'Pause()', Pause, 'vmf', 'Pause the animation loop'),
    input('Tick', 'Tick(bool)', (inputData) => {
        const value = inputData.caller ? inputData.caller.GetEntityName() === 'true' : true;
        Tick(value);
    }, 'vmf', 'Set ticking state'),
    input('OnDamaged', 'OnDamaged()', onDamaged, 'vmf', 'Apply knockback to owner'),
    input('SetKnockback_1', 'SetKnockback(1)', (inputData) => setKnockback(inputData, 1), 'vmf', 'Set KNOCKBACK to 1'),
    input('SetKnockback_100', 'SetKnockback(100)', (inputData) => setKnockback(inputData, 100), 'vmf', 'Set KNOCKBACK to 100'),
    input('SetKnockback_160', 'SetKnockback(160)', (inputData) => setKnockback(inputData, 160), 'vmf', 'Set KNOCKBACK to 160'),
    input('SetKnockback_200', 'SetKnockback(200)', (inputData) => setKnockback(inputData, 200), 'vmf', 'Set KNOCKBACK to 200'),
    input('SetKnockback_300', 'SetKnockback(300)', (inputData) => setKnockback(inputData, 300), 'vmf', 'Set KNOCKBACK to 300'),
    input('SetKnockback_500', 'SetKnockback(500)', (inputData) => setKnockback(inputData, 500), 'vmf', 'Set KNOCKBACK to 500'),
    input('SetHandler_1', 'SetHandler(1)', (inputData) => setHandler(inputData, 1), 'vmf', 'Bind to less_demon_mdl'),
    input('SetHandler_2', 'SetHandler(2)', (inputData) => setHandler(inputData, 2), 'vmf', 'Bind to skeleton_mdl'),
    input('SetHandler_3', 'SetHandler(3)', (inputData) => setHandler(inputData, 3), 'vmf', 'Bind to nightmare_mdl'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[skin_item_animation] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
