...
const SCRIPT_PREFIX = 'zm_revenge_item';
// 全局状态
let hit_count = 1000;
let current = 1000;
let mode = 0;
function onHit(inputData) {
    //printl(`[ZM Revenge] onHit, current=${current}`)
    if (current > 0) {
        current--;
    }
    else {
        // 触发粒子效果
        EntFire('zm_revenge_pcf', 'Start');
        mode = 1;
        current = hit_count;
        //printl(`[ZM Revenge] mode set to 1, current reset to ${current}`)
    }
}
function increaseDmg() {
    EntFire('revenge_hurt_1', 'SetDamage', '150', 0.10);
}
function onUsed(inputData) {
    // 重置计数器
    current = hit_count;
    //printl(`[ZM Revenge] onUsed, current reset to ${current}, mode=${mode}`)
    if (mode == 1) {
        // 暴击模式，生成威力加强版
        EntFire('zm_revenge_maker', 'ForceSpawn', '', 0.0);
        increaseDmg();
        mode = 0;
        //printl('[ZM Revenge] used in mode 1, spawned enhanced item')
    }
    else {
        // 普通模式
        mode = 0;
        EntFire('zm_revenge_maker', 'ForceSpawn', '', 0.0);
        EntFire('revenge_hurt_1', 'SetDamage', '10', 0.10);
        //printl('[ZM Revenge] used in mode 0, spawned normal item')
    }
}
const EXTERNAL_INPUT_ALIASES = [
    input('OnHit', 'OnHit()', onHit, 'vmf', 'Decrement counter, trigger mode 1 when zero'),
    input('OnUsed', 'OnUsed()', onUsed, 'vmf', 'Use the item, spawn revenge item'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[zm_revenge_item] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
