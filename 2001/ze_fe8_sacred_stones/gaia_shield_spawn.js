...
const SCRIPT_PREFIX = 'gaia_shield_spawn';
// ===================================\\
// 配置常量
// ===================================\\
const SHIELD_STRING = 'level1_shield_maker_';
// ===================================\\
// 核心函数
// ===================================\\
function addShields(a) {
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        if (i === a) {
            EntFire(temp, 'KeyValues', 'EntityTemplate level1_shield_maker_temp2');
            EntFire(temp, 'ForceSpawn', '', 0.5);
        }
        else {
            EntFire(temp, 'KeyValues', 'EntityTemplate level1_shield_maker_temp1');
            EntFire(temp, 'ForceSpawn', '', 0.5);
        }
    }
}
...
const EXTERNAL_INPUT_ALIASES = [
    input('PrepareShields', 'PrepareShields()', prepareShields, 'vmf', 'Randomly select one shield as solution'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[gaia_shield_spawn] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
