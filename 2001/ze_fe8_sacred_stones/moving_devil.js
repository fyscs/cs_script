...
const SCRIPT_PREFIX = 'moving_devil';
// ===================================\\
// 配置常量
// ===================================\\
const TICKRATE = 0.05;
...
function initEntities(suffix) {
    if (!self || !self.IsValid()) {
        printl('[MovingDevil] initEntities: self is invalid');
        return;
    }
    const pcfName = `devil_bring_pcf_${suffix}`;
    const hitboxName = `devil_bring_hitbox_${suffix}`;
    const trigName = `devil_bring_trig_${suffix}`;
    pcf = Instance.FindEntityByName(pcfName);
    hitbox = Instance.FindEntityByName(hitboxName);
    trig = Instance.FindEntityByName(trigName);
    ignoreList = [];
    if (self && self.IsValid())
    ...
    if (hitbox && hitbox.IsValid())
        ignoreList.push(hitbox);
    if (trig && trig.IsValid())
        ignoreList.push(trig);
    printl(`[MovingDevil] initEntities: self=${self ? 'ok' : 'null'}, pcf=${pcf ? 'found' : 'null'}, hitbox=${hitbox ? 'found' : 'null'}, trig=${trig ? 'found' : 'null'}`);
}
// ===================================\\
// 输入处理
// ===================================\\
function SetSelf(inputData) {
    const { caller } = inputData;
    if (!caller || !caller.IsValid()) {
        printl('[MovingDevil] SetSelf: caller invalid');
        return;
    }
    self = caller;
    printl(`[MovingDevil] SetSelf: self set to ${caller.GetEntityName()}`);
    const suffix = getSuffix(caller.GetEntityName());
    if (suffix) {
        initEntities(suffix);
    }
    else {
        printl('[MovingDevil] SetSelf: no suffix found, ignoring only self');
        ignoreList = [self];
    }
    printl(`[MovingDevil] SetSelf: ignoreList size = ${ignoreList.length}`);
}
function Start() {
    printl('[MovingDevil] Start: entered');
    if (!self || !self.IsValid()) {
        printl('[MovingDevil] Start: self not set, call SetSelf first');
        return;
    }
    if (ticking) {
        printl('[MovingDevil] Start: already running');
        return;
    }
    ticking = true;
    SearchTarget();
    printl(`[MovingDevil] Start: started, initial target=${target ? getClassname(target) : 'none'}`);
    Tick();
}
function Stop() {
    printl('[MovingDevil] Stop: entered');
    if (!ticking) {
        printl('[MovingDevil] Stop: already stopped');
        return;
    }
    ticking = false;
    printl('[MovingDevil] Stop: stopped');
}
function Status() {
    printl(`[MovingDevil] Status: ticking=${ticking}, self=${self ? 'ok' : 'null'}, target=${target ? getClassname(target) : 'null'}`);
}
// ===================================\\
// 主逻辑
// ===================================\\
function Tick() {
    printl(`[MovingDevil] Tick: entered, ticking=${ticking}`);
    if (!ticking) {
        return;
    }
    scheduleScript(SCRIPT_PREFIX, () => Tick(), TICKRATE, null, null);
    if (!self || !self.IsValid()) {
        printl('[MovingDevil] Tick: self invalid, stopping');
        Stop();
        return;
    }
    tickCounter++;
    if (tickCounter % 20 === 0) {
        printl(`[MovingDevil] Tick #${tickCounter}: target=${target ? getClassname(target) : 'null'}`);
    }
    if (!target || !target.IsValid() || target.GetHealth() <= 0 || getTeam(target) !== 3) {
        SearchTarget();
        if (!target) {
            return;
        }
    }
    const selfPos = getOrigin(self);
    const targetPos = getOrigin(target);
    const dirVec = Vector(targetPos.x - selfPos.x, targetPos.y - selfPos.y, targetPos.z - selfPos.z);
    const len = Math.sqrt(dirVec.x * dirVec.x + dirVec.y * dirVec.y + dirVec.z * dirVec.z);
    if (len < 1) {
        return;
    }
    const dir = VectorScale(dirVec, 1 / len);
    const newPos = VectorAdd(selfPos, VectorScale(dir, SPEED));
    setOrigin(self, newPos);
}
function SearchTarget() {
    printl('[MovingDevil] SearchTarget: entered');
    if (!self || !self.IsValid()) {
        printl('[MovingDevil] SearchTarget: self invalid');
        target = null;
        return;
    }
    const origin = getOrigin(self);
    const startPos = origin;
    const candidates = [];
    const players = Instance.FindEntitiesByClass('player');
    printl(`[MovingDevil] SearchTarget: scanning ${players.length} players`);
    for (const p of players) {
        if (!p.IsValid())
            continue;
        if (getClassname(p) !== 'player' || getTeam(p) !== 3 || p.GetHealth() <= 0)
            continue;
        const dist = distance(origin, getOrigin(p));
        if (dist > SEARCH_RADIUS) {
            continue;
        }
        const endPos = VectorAdd(getOrigin(p), Vector(0, 0, TARGET_OFFSET_Z));
        const trace = TraceLine(startPos, endPos, ignoreList.length > 0 ? ignoreList : [self], true);
        if (trace >= 1.0) {
            candidates.push(p);
            printl(`[MovingDevil] SearchTarget: candidate found at distance ${dist.toFixed(1)}`);
        }
    }
    if (candidates.length === 0) {
        target = null;
        printl('[MovingDevil] SearchTarget: no visible CT found (players ignored)');
        return;
    }
    let nearest = candidates[0];
    let minDist = distance(origin, getOrigin(nearest));
    for (const c of candidates) {
        const d = distance(origin, getOrigin(c));
        if (d < minDist) {
            minDist = d;
            nearest = c;
        }
    ...
    target = nearest;
    printl(`[MovingDevil] SearchTarget: target set to ${getClassname(target)} at distance ${minDist.toFixed(1)}`);
}
// ===================================\\
// 输入别名注册
// ===================================\\
const EXTERNAL_INPUT_ALIASES = [
    input('SetSelf', 'SetSelf()', SetSelf, 'vmf', 'Set devil_moving as caller'),
    input('Start', 'Start()', Start, 'vmf', 'Start chasing target'),
    input('Stop', 'Stop()', Stop, 'vmf', 'Stop chasing (hold position)'),
    input('Status', 'Status()', Status, 'vmf', 'Print debug status'),
];
...
// ===================================\\
installScheduler();
Instance.Msg('[moving_devil] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
