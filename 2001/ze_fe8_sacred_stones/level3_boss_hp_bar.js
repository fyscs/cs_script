...
const SCRIPT_PREFIX = 'level3_boss_hp_bar';
// ===================================\\
// 状态变量
// ===================================\\
let current_Hp = 0;
...
// 核心函数
// ===================================\\
function scaleHp_500() {
    current_Hp += 500;
    printl(`[Level3BossHP] scaleHp_500: current_Hp = ${current_Hp}`);
}
function splitHp() {
    eachBar = Math.floor(current_Hp / 3);
    printl(`[Level3BossHP] splitHp: eachBar = ${eachBar}`);
}
function subtractHp() {
    current_Hp -= 1;
    printl(`[Level3BossHP] subtractHp: current_Hp = ${current_Hp}`);
    checkIfBreak();
}
function checkIfBreak() {
    ...
    if (bar === 0)
        return;
    if (hp <= bar * 0) {
        EntFire('queen_crystal_1', 'Disable');
        return;
    }
    if (hp <= bar * 1) {
        EntFire('queen_crystal_2', 'Disable');
        return;
    }
    if (hp <= bar * 2) {
        EntFire('queen_crystal_3', 'Disable');
        return;
    }
}
// ===================================\\
...
const EXTERNAL_INPUT_ALIASES = [
    input('ScaleHp_500', 'ScaleHp_500()', scaleHp_500, 'vmf', 'Add 500 HP to boss'),
    input('SplitHp', 'SplitHp()', splitHp, 'vmf', 'Calculate each bar (HP / 3)'),
    input('SubtractHp', 'SubtractHp()', subtractHp, 'vmf', 'Subtract 1 HP and check break'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[level3_boss_hp_bar] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
