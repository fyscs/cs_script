...
const SCRIPT_PREFIX = 'level1_boss_hp_script';
// ===================================\\
// 状态变量
// ===================================\\
let current_Hp = 0;
...
// 辅助函数
// ===================================\\
// 更新显示（使用固定实体名 level1_boss_hp_text）
function updateDisplay() {
    EntFire('level1_boss_hp_text', 'SetMessage', `Guardian Hp: ${current_Hp}`);
}
// ===================================\\
// 核心函数
// ===================================\\
function scaleHp(scaler) {
    current_Hp += scaler;
    max_hp += scaler;
    printl(`[Level1BossHP] scaleHp: current_Hp=${current_Hp}, max_hp=${max_hp}`);
    updateDisplay();
}
function subtractHp() {
    current_Hp -= 1;
    printl(`[Level1BossHP] subtractHp: current_Hp=${current_Hp}`);
    if (current_Hp <= 0) {
        // Death sequences
        EntFire('level1_boss_hitbox', 'Kill');
        EntFire('actual_boss_mdl', 'SetAnimationNotLooping', 'golem_low_health');
        EntFire('level1_boss_nuke_relay', 'Kill');
        EntFire('level1_boss_nuke', 'Kill');
        EntFire('death_sound', 'PlaySound');
        EntFire('level1_killallboss_atk', 'Trigger');
        EntFire('level1_boss_main_timer', 'Disable');
        EntFire('level1_boss_model', 'Disable', '', 4.00);
        EntFire('actual_boss_mdl', 'SetAnimationLooping', 'golem_low_death', 4.00);
        EntFire('Server', 'Command', 'say <<<Congratulations, Advancing to next chapter...>>>', 5.00);
        EntFire('level_counter', 'SetValue', '5', 5.00);
        EntFire('zr_toggle_respawn', 'Disable', '', 5.00);
        EntFire('spawn_nuke', 'Enable', '', 6.00);
        EntFire('boss_t_nuke', 'Enable', '', 6.00);
        EntFire('Map_Round_End', 'RunScriptInput', 'KillAllT', 6.50);
        EntFire('actual_boss_mdl', 'Disable', '', 4.00);
        EntFire('level1_boss_hp_text', 'Kill', '', 1.00);
        return;
    }
    updateDisplay();
    check();
...
function check() {
    if (current_Hp === Math.floor(max_hp / 2)) {
        EntFire('Server', 'Command', 'say <<<All damages and frequencies are now increased>>>');
        EntFire('level1_boss_earthquake_hurt', 'SetDamage', '30');
        EntFire('level1_boss_main_timer', 'RefireTime', '15');
        printl('[Level1BossHP] Half HP reached, damage increased');
    }
}
// ===================================\\
// 输入别名注册
...
const EXTERNAL_INPUT_ALIASES = [
    input('ScaleHp_515', 'ScaleHp(515)', () => scaleHp(515), 'vmf', 'Add 515 HP'),
    input('SubtractHp', 'SubtractHp()', subtractHp, 'vmf', 'Subtract 1 HP and check for death/half HP'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[level1_boss_hp_script] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
