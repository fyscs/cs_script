...
const SCRIPT_PREFIX = 'level5_boss_controller';
// ===================================\\
// 配置常量
// ===================================\\
...
let canhit = 1;
let each_bar = 0;
let formortiis_can_heal = true;
// ===================================\\
// 核心函数
// ===================================\\
function scaleHp(inputData, mode) {
    // 0 = more hp, weaker attack
    // 1 = less hp, stronger attack
    if (mode === 0) {
        const add = Math.floor(2200 * HP_MULTIPLIER);
        formortiis_hp += add;
        EntFire('formortiis_hp', 'Add', add.toString());
    }
    else {
        const add = Math.floor(1800 * HP_MULTIPLIER);
        formortiis_hp += add;
        EntFire('formortiis_hp', 'Add', add.toString());
    }
}
function halveBossHp() {
    formortiis_hp = Math.floor(formortiis_hp / 2);
    EntFire('formortiis_hp', 'SetValue', formortiis_hp.toString());
    each_bar = Math.floor(formortiis_hp / 4);
}
function onHit() {
    if (canhit === 1) {
        formortiis_hp -= 2;
        EntFire('formortiis_hp', 'Subtract', '2');
        printl(`[Level5Boss] HP: ${formortiis_hp}`);
        checkIfBreak();
        if (formortiis_hp <= 0) {
            EntFire('formortiis_fire_1', 'Stop');
        }
    }
    else {
        const add = Math.floor(10 * HP_MULTIPLIER);
        formortiis_hp += add;
        EntFire('formortiis_hp', 'Add', add.toString());
        printl(`[Level5Boss] HP (restore): ${formortiis_hp}`);
        checkIfRestore();
    }
}
function checkIfBreak() {
    const hp = formortiis_hp;
    const bar = each_bar;
    if (bar === 0)
        return;
    if (hp <= bar * 0) {
    ...
    if (hp <= bar * 1) {
        EntFire('formortiis_fire_2', 'Stop');
        return;
    }
    if (hp <= bar * 2) {
        if (formortiis_can_heal) {
            formortiis_can_heal = false;
            const heal = Math.floor(2750 * HP_MULTIPLIER);
            formortiis_hp += heal;
            EntFire('formortiis_fire_3', 'Stop');
            EntFire('formortiis', 'Skin', '1');
            EntFire('formortiis', 'Color', '255 0 6');
            EntFire('Server', 'Command', 'say <<<Hah, do you really think your guns can hurt me?>>>', 1.00);
            EntFire('Server', 'Command', 'say <<<***All damage increased! Extra attack combo frequency increased!***>>>', 2.00);
            EntFire('Server', 'Command', 'say <<<***Formortiis has healed himself***>>>', 3.00);
            EntFire('formortiis_hp', 'Add', heal.toString());
            EntFire('formortiis_light_hurt', 'SetDamage', '85');
            EntFire('contagious_mist_trig', 'SetDamage', '30');
            EntFire('contagious_mist_trig2', 'SetDamage', '30');
            EntFire('devil_bring_timer', 'RefireTime', '1');
        }
        else {
            EntFire('formortiis_fire_3', 'Stop');
        }
    ...
    if (hp <= bar * 3) {
        EntFire('formortiis_fire_4', 'Stop');
        return;
    }
}
function checkIfRestore() {
    ...
    if (bar === 0)
        return;
    if (hp <= bar * 0) {
        // EntFire('formortiis_fire_1', 'Start')
        return;
    }
    if (hp <= bar * 1) {
        EntFire('formortiis_fire_2', 'Start');
        return;
    }
    if (hp <= bar * 2) {
        EntFire('formortiis_fire_3', 'Start');
        return;
    }
    if (hp <= bar * 3) {
        EntFire('formortiis_fire_4', 'Start');
        return;
    }
}
function setHitVar(inputData, value) {
    canhit = value;
    printl(`[Level5Boss] setHitVar: canhit = ${canhit}`);
}
function addHp(inputData, value) {
    const add = Math.floor(value * HP_MULTIPLIER);
    formortiis_hp += add;
    EntFire('formortiis_hp', 'Add', add.toString());
    printl(`[Level5Boss] addHp: HP = ${formortiis_hp}`);
}
// ===================================\\
// 输入别名注册
// ===================================\\
const EXTERNAL_INPUT_ALIASES = [
    input('ScaleHp_0', 'ScaleHp(0)', (inputData) => scaleHp(inputData, 0), 'vmf', 'Scale HP mode 0 (more HP, weaker attack)'),
    input('ScaleHp_1', 'ScaleHp(1)', (inputData) => scaleHp(inputData, 1), 'vmf', 'Scale HP mode 1 (less HP, stronger attack)'),
    input('HalveBossHp', 'HalveBossHp()', halveBossHp, 'vmf', 'Halve boss HP and recalc bars'),
    input('OnHit', 'OnHit()', onHit, 'vmf', 'Hit boss (subtract 2 HP or restore if canhit=0)'),
    input('SetHitVar_0', 'SetHitVar(0)', (inputData) => setHitVar(inputData, 0), 'vmf', 'Set canhit to 0 (heal on hit)'),
    input('SetHitVar_1', 'SetHitVar(1)', (inputData) => setHitVar(inputData, 1), 'vmf', 'Set canhit to 1 (damage on hit)'),
    input('AddHp_10', 'AddHp(10)', (inputData) => addHp(inputData, 10), 'vmf', 'Add 10 HP (multiplied by 1.3)'),
    input('AddHp_150', 'AddHp(150)', (inputData) => addHp(inputData, 150), 'vmf', 'Add 100 HP (multiplied by 1.3)'),
    input('AddHp_1000', 'AddHp(1000)', (inputData) => addHp(inputData, 1000), 'vmf', 'Add 1000 HP (multiplied by 1.3)'),
    input('AddHp_2750', 'AddHp(2750)', (inputData) => addHp(inputData, 2750), 'vmf', 'Add 2750 HP (multiplied by 1.3)'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[level5_boss_controller] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
