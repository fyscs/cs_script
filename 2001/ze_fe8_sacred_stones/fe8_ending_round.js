...
const SCRIPT_PREFIX = 'fe8_ending_round';
function KillAllT() {
    ...
    for (const controller of all) {
        ...
        Instance.EntFireAtTarget({ target: player, input: 'SetHealth', value: '-1', delay: 0.1 });
    }
}
Instance.OnRoundEnd(() => {
    EntFire('Clear_Script', 'Trigger');
});
const EXTERNAL_INPUT_ALIASES = [
    input('KillAllT', 'KillAllT()', KillAllT, 'vmf', 'Kill all T players and disable respawn'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[fe8_ending_round] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
