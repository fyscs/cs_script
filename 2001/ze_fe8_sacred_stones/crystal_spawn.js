...
const SCRIPT_PREFIX = 'crystal_spawn';
// ===================================\\
// 状态变量
// ===================================\\
let isUsed = [0, 1, 2, 3, 4, 5];
let copyUsed = [0, 0, 0, 0, 0, 0];
let current = 1;
...
function prepare() {
    // Copy isUsed to copyUsed
    copyUsed = [...isUsed];
    // Shuffle isUsed multiple times
    for (let i = 0; i < isUsed.length; i++) {
        shuffle(isUsed);
    }
    printl('---------------------------------');
    for (let j = 0; j < isUsed.length; j++) {
        printl(`[CrystalSpawn] isUsed[${j}] = ${isUsed[j]}`);
    }
}
function setTemplatesAndSpawn() {
    if (current > 6) {
        printl('[CrystalSpawn] setTemplatesAndSpawn: all crystals spawned');
        return;
    }
    const temp = `level3_shield_maker_${current}`;
    const val = isUsed.pop();
    if (val === undefined) {
        printl('[CrystalSpawn] setTemplatesAndSpawn: no more values');
        return;
    }
    printl(`[CrystalSpawn] shield maker #: ${temp}`);
    printl(`[CrystalSpawn] Value #: ${val}`);
    switch (val) {
        case 0:
            EntFire(temp, 'KeyValues', 'EntityTemplate level3_crystal_template_a');
            EntFire(temp, 'ForceSpawn', '', 0.5);
            break;
        case 1:
            EntFire(temp, 'KeyValues', 'EntityTemplate level3_crystal_template_b');
            EntFire(temp, 'ForceSpawn', '', 0.5);
            break;
        case 2:
            EntFire(temp, 'KeyValues', 'EntityTemplate level3_crystal_template_c');
            EntFire('crystal_spear_temp', 'ForceSpawn', '', 0.5);
            EntFire(temp, 'ForceSpawn', '', 0.5);
            break;
        case 3:
            EntFire(temp, 'KeyValues', 'EntityTemplate level3_crystal_template_d');
            EntFire(temp, 'ForceSpawn', '', 0.5);
            break;
        case 4:
            EntFire(temp, 'KeyValues', 'EntityTemplate level3_crystal_template_e');
            EntFire(temp, 'ForceSpawn', '', 0.5);
            break;
        case 5:
            EntFire(temp, 'KeyValues', 'EntityTemplate level3_crystal_template_f');
            EntFire(temp, 'ForceSpawn', '', 0.5);
            break;
        default:
            printl('[CrystalSpawn] No Spawn');
    }
    current += 1;
}
function refresh() {
    current = 1;
    isUsed = [...copyUsed];
    printl('[CrystalSpawn] refresh: isUsed restored');
}
// ===================================\\
// 输入别名注册
// ===================================\\
const EXTERNAL_INPUT_ALIASES = [
    input('Prepare', 'Prepare()', prepare, 'vmf', 'Initialize and shuffle crystal array'),
    input('SetTemplatesAndSpawn', 'SetTemplatesAndSpawn()', setTemplatesAndSpawn, 'vmf', 'Spawn next crystal from shuffled array'),
    input('Refresh', 'Refresh()', refresh, 'vmf', 'Restore isUsed from copyUsed and reset current'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[crystal_spawn] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
