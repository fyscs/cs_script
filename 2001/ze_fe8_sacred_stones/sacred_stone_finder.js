...
const SCRIPT_PREFIX = 'sacred_stone_finder';
// ===================================\\
// 状态变量
// ===================================\\
let sacred = null;
...
// 核心函数
// ===================================\\
function findSacredStone(inputData) {
    const { caller } = inputData;
    if (!caller || !caller.IsValid()) {
        printl('[SacredStoneFinder] findSacredStone: caller invalid');
        return;
    }
    const origin = getOrigin(caller);
    sacred = findByNameNearest('sacred_stone_mdl', origin, 3900);
    if (sacred && sacred.IsValid()) {
        printl('[SacredStoneFinder] Found sacred stone');
        EntFire('sacred_stone_exists', 'Trigger');
    }
    else {
        printl('[SacredStoneFinder] Sacred stone not found');
    }
}
function findShip1(inputData) {
    const { caller } = inputData;
    if (!caller || !caller.IsValid()) {
        printl('[SacredStoneFinder] findShip1: caller invalid');
        return;
    }
    const origin = getOrigin(caller);
    ship_1 = findByNameNearest('zombie_ship', origin, 3200);
    if (!ship_1 || !ship_1.IsValid()) {
        EntFire('human_ship', 'Break', '', 1.00);
        EntFire('level_counter', 'SetValue', '5');
        EntFire('Server', 'Command', 'say ***Ship bugged...moving to next chapter***');
        printl('[SacredStoneFinder] Ship1 not found, advancing');
    }
    else {
        printl('[SacredStoneFinder] Ship1 found');
    }
}
function findShip2(inputData) {
    const { caller } = inputData;
    if (!caller || !caller.IsValid()) {
        printl('[SacredStoneFinder] findShip2: caller invalid');
        return;
    }
    const origin = getOrigin(caller);
    ship_2 = findByNameNearest('zombie_ship2', origin, 3200);
    if (!ship_2 || !ship_2.IsValid()) {
        EntFire('human_ship', 'Break', '', 1.00);
        EntFire('level_counter', 'SetValue', '5');
        EntFire('Server', 'Command', 'say ***Ship bugged...moving to next chapter***');
        printl('[SacredStoneFinder] Ship2 not found, advancing');
    }
    else {
        printl('[SacredStoneFinder] Ship2 found');
    }
}
const EXTERNAL_INPUT_ALIASES = [
    input('FindSacredStone', 'FindSacredStone()', findSacredStone, 'vmf', 'Find sacred stone within 3900 units'),
    input('FindShip1', 'FindShip1()', findShip1, 'vmf', 'Find zombie_ship within 3200 units'),
    input('FindShip2', 'FindShip2()', findShip2, 'vmf', 'Find zombie_ship2 within 3200 units'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
Instance.Msg('[sacred_stone_finder] script loaded with ' + EXTERNAL_INPUT_ALIASES.length + ' inputs');
