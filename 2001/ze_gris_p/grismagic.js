    ...
    const interval = setInterval(() => {
        if (CLEAR_ALL_INTERVAL || !HEAL) {
            ...
            for (const heal of heals) {
                for (const player of players) {
                    if (player?.IsValid() && player.IsAlive() && player.GetTeamNumber() == 3 && Vector3Utils.distance(player.GetAbsOrigin(), heal.GetAbsOrigin()) < HEAL_RADIUS && player.GetHealth() < 100 && player.GetHealth() > 0) {
