...
//     if (event.text === "admin") {
//         pawn.Teleport({ position: new Vec3(-88, -108, -86), angles: new Euler(0, 270, 0) })
//     }
// })
const script_overkill = "script_overkill";
let level = 1;
let warmup = true;
let first_round = true;
let time = 0;
const boss_tp_dest = new Vec3(-1280, 11904, 14384);
...
Instance.OnRoundStart(async () => {
    ...
    if (first_round) {
        first_round = false;
        Instance.ServerCommand("say WARMUP");
        await Instance.Delay(5);
        Instance.ServerCommand("say WARMUP");
        await Instance.Delay(5);
        Instance.ServerCommand("say WARMUP");
        await Instance.Delay(5);
        ...
        Instance.ServerCommand("say WARMUP");
        await Instance.Delay(5);
        Instance.ServerCommand("say Starting Game...");
    ...
    for (const player of players) {
        ...
        if (knife !== undefined) {
            const class_name = knife.GetClassName();
            if (class_name === "weapon_knife") {
    ...
    // })
    // Instance.ConnectOutput(math_counter2, "OutValue", (inputData) => {
    //     print(inputData.value);
    // })
});
...
Instance.OnScriptInput("TextHeal", ({ activator }) => {
    if (activator instanceof CSPlayerPawn) {
        ...
        setTextPlayer("Healing and Unlimited Ammo\n45 Seconds Cooldown\nUnlimited Use", controller, heal_text);
    }
});
const molotov_text = {
    channel: 5,
...
Instance.OnScriptInput("TextMolotov", ({ activator }) => {
    if (activator instanceof CSPlayerPawn) {
        const controller = activator.GetPlayerController();
        setTextPlayer("Burns and Slows Zombies\n30 Seconds Cooldown\n3 Uses Only", controller, molotov_text);
    }
});
const dogma_text = {
    channel: 5,
...
Instance.OnScriptInput("TextDogma", ({ activator }) => {
    if (activator instanceof CSPlayerPawn) {
        const controller = activator.GetPlayerController();
        setTextPlayer("Dogma Resistance\nDestroys the Overkill\n1 Use Only", controller, dogma_text);
    }
});
const freeze_text = {
    channel: 5,
...
Instance.OnScriptInput("TextFreeze", ({ activator }) => {
    if (activator instanceof CSPlayerPawn) {
        const controller = activator.GetPlayerController();
        setTextPlayer("Freeze and Damage Humans\n60 Seconds Cooldown\nUnlimited Use", controller, freeze_text);
    }
});
const map_by_text = {
    channel: 2,
...
Instance.OnScriptInput("TextMapBy", () => {
...
Instance.OnScriptInput("ColdGreenEyes", async () => {
    ...
    setText("Ever changing", { ...cold_green_eyes_lyrics, duration: 2 });
    await Instance.Delay(2.4);
    if (startingRound !== roundGeneration)
        return;
    setText("The knowing feeling", { ...cold_green_eyes_lyrics, duration: 2.5 });
    await Instance.Delay(2.75);
    if (startingRound !== roundGeneration)
        return;
    setText("Could make you fall from the dreamy skies", { ...cold_green_eyes_lyrics, duration: 5, fadeOutTime: 1 });
    await Instance.Delay(6.75);
    if (startingRound !== roundGeneration)
        return;
    setText("Suddenly the deepness can get true", { ...cold_green_eyes_lyrics, duration: 2.5 });
    await Instance.Delay(2.9);
    if (startingRound !== roundGeneration)
        return;
    setText("Where you live a life without a clue", { ...cold_green_eyes_lyrics, duration: 3 });
...
Instance.OnScriptInput("ColdGreenEyes2", async () => {
    ...
    if (startingRound !== roundGeneration)
        return;
    setText("Realize I fall along with you", { ...cold_green_eyes_lyrics, duration: 3 });
...
async function showOverkill(time) {
    if (!Number.isFinite(time)) {
        throw new TypeError('Overkill effect time must be a finite number.');
    }
    if (time <= 0) {
        return;
    }
    ...
    let remainingPulseTime = timing.holdTime;
    let dimmed = true;
    while (remainingPulseTime > 0) {
        const pulseFadeTime = Math.min(OVERKILL_PULSE_FADE_TIME, remainingPulseTime);
        setHudTextFadeDuration(pulseFadeTime, channel);
        await Instance.Delay(FADE_START_DELAY);
        if (!isOverkillEffectCurrent(startingRound, effectRequestId, channel, textRequestId)) {
            return;
        }
        for (const panelId of getChannelTextPanelIds(channel)) {
            getHudLayout().SetHasClass(panelId, OVERKILL_DIMMED_CLASS, dimmed);
        }
        await Instance.Delay(pulseFadeTime);
        if (!isOverkillEffectCurrent(startingRound, effectRequestId, channel, textRequestId)) {
            return;
        }
        remainingPulseTime -= pulseFadeTime;
        dimmed = !dimmed;
    }
...
function isOverkillEffectCurrent(startingRound, effectRequestId, channel, textRequestId) {
    return startingRound === roundGeneration
        && effectRequestId === overkillEffectRequestId
        && textRequestId === channelStates[channel].textRequestId;
}
async function showKillThemAll() {
    const startingRound = roundGeneration;
...
function clearKillThemAllImages() {
    ++imageSequenceRequestId;
    hideKillThemAllImages();
}
Instance.OnScriptInput("Overkill_L1", () => {
    setText("Running full system diagnostics", overkill_lyrics);
});
Instance.OnScriptInput("Overkill_L2", () => {
    setText("Machine status: Fully operational", overkill_lyrics);
});
Instance.OnScriptInput("Overkill_L3", () => {
    setText("System: Online", { ...overkill_lyrics, duration: 2 });
});
Instance.OnScriptInput("Overkill_L4", () => {
    setText("All stations are now enabled", overkill_lyrics);
});
Instance.OnScriptInput("Overkill_L5", () => {
    setText("Target: Dogma Resistance", { ...overkill_lyrics, duration: 2.5 });
});
Instance.OnScriptInput("Overkill_L6", () => {
    setText("Operation Overkill", { ...overkill_lyrics, duration: 1.75 });
});
Instance.OnScriptInput("ShowOverkill43", () => {
    showOverkill(43);
});
Instance.OnScriptInput("ShowOverkill9", () => {
    showOverkill(9);
});
Instance.OnScriptInput("Overkill_L7", () => {
    setText("Kill Them All", { ...overkill_lyrics, color: 'red', duration: 2 });
});
Instance.OnScriptInput("ShowKillThemAll", () => {
    void showKillThemAll();
});
Instance.OnScriptInput("Overkill_L8", () => {
...
Instance.OnScriptInput("StartElevatorTimer", () => {
    startTimer('elevator', 48);
});
Instance.OnScriptInput("StartAtomicTimer", () => {
    startTimer('atomic', 33);
});
...
function getTimerText(timerKind, seconds) {
    ...
    switch (timerKind) {
        case 'maze':
            return `Exit maze door closing in ${formattedTime}`;
        case 'elevator':
            return `Exit elevator leaving in ${formattedTime}`;
        case 'atomic':
            return `Atomic bomb arriving in ${formattedTime}`;
        case 'tower':
            return `Tower entrance closing in ${formattedTime}`;
    }
}
...
Instance.OnScriptInput("txt_earth", () => {
    setText("Earth: Above The Surface", hint_text2);
});
Instance.OnScriptInput("hoursLater", () => {
    setText("Several hours later...", hint_text2);
});
Instance.OnScriptInput("txt_earth2", () => {
    setText("Earth: Surface", hint_text2);
});
Instance.OnScriptInput("txt_earth3", () => {
    setText("Earth: Underground", hint_text2);
});
Instance.OnScriptInput("txt_earth2_destruction", () => {
    setText("Earth: Surface (Destruction)", hint_text2);
});
Instance.OnScriptInput("txt_year", () => {
    setText("Year | 3230", { ...hint_text2, duration: 4.5 });
});
const random_years = [
    "Year | 3134",
    "Year | �622",
...
Instance.OnScriptInput("RandomYear", () => {
    const year = random_years[getRandomInt(0, random_years.length - 1)];
    setText(year, { ...hint_text2, duration: 4.5 });
});
Instance.OnScriptInput("txt_year1", () => {
    setText("Year | 3�55", { ...hint_text2, duration: 4.5 });
});
Instance.OnScriptInput("txt_year2", () => {
    setText("Year | 3��0", { ...hint_text2, duration: 4.5 });
});
Instance.OnScriptInput("txt_year3", () => {
    setText("�e�r | �4�3", { ...hint_text2, duration: 4.5 });
});
Instance.OnScriptInput("txt_year4", () => {
    setText("���r | ���0", { ...hint_text2, duration: 4.5 });
});
Instance.OnScriptInput("txt_year5", () => {
    setText("���� | ����", { ...hint_text2, duration: 4.5 });
});
Instance.OnScriptInput("txt_year6", () => {
    setText("���� | 3230", { ...hint_text2, duration: 4.5 });
});
Instance.OnScriptInput("ending_bad", () => {
    setText("Ending | Bad", hint_text2);
});
Instance.OnScriptInput("ending_good", () => {
    setText("���� | Good", hint_text2);
});
Instance.OnScriptInput("ending_true", () => {
    setText("���� | ����", hint_text2);
});
Instance.OnScriptInput("txt_ending_true1", () => {
    setText("| Once upon a time |", hint_text3);
});
Instance.OnScriptInput("txt_ending_true2", () => {
    setText("| There was someone |", hint_text3);
});
Instance.OnScriptInput("txt_ending_true3", () => {
    setText("| Who has ability to � through the � |", hint_text3);
});
Instance.OnScriptInput("txt_ending_true4", () => {
    setText("| His � becomes � everytime he � |", hint_text3);
});
Instance.OnScriptInput("hint1", () => {
    setText("Explosion sounds...", hint_text1);
});
Instance.OnScriptInput("hint2", () => {
    setText("I need to find a place to take a rest.", hint_text1);
});
Instance.OnScriptInput("hint3", () => {
    setText("That secret weapon is hidden somewhere", hint_text1);
});
Instance.OnScriptInput("hint4", () => {
    setText("I have to find it before anyone else.", hint_text1);
});
Instance.OnScriptInput("hint5", () => {
    setText("The sky is getting dark..", hint_text1);
});
Instance.OnScriptInput("hint6", () => {
    setText("The Overkill has been destroyed", hint_text1);
});
Instance.OnScriptInput("hint7", () => {
    setText("The world can have its peace now", hint_text1);
});
Instance.OnScriptInput("hint8", () => {
    setText("But this is not right...", hint_text1);
});
Instance.OnScriptInput("hint9", () => {
    setText("The �� is not the correct ��", hint_text1);
});
Instance.OnScriptInput("hint10", () => {
    setText("I have to reset the ��, and comeback to the very beginning", hint_text1);
});
Instance.OnScriptInput("hint11", () => {
    setText("The Overkill is still roaming around the world", hint_text1);
});
Instance.OnScriptInput("hint12", () => {
    setText("But it doesn't matter right now", hint_text1);
});
Instance.OnScriptInput("hint13", () => {
    setText("At least I'm safe down here forever...", hint_text1);
});
Instance.OnScriptInput("hint14", () => {
    setText("Is this really the correct path I walk into", hint_text1);
});
Instance.OnScriptInput("hint15", () => {
    setText("Or should I � again.", hint_text1);
});
Instance.OnScriptInput("hint16", () => {
    setText("Maybe I'm the only one who can � the �", hint_text1);
});
Instance.OnScriptInput("hint17", () => {
    setText("To wield this much �", hint_text1);
});
Instance.OnScriptInput("hint18", () => {
    setText("And to create this destiny, it has to be my", hint_text1);
});
Instance.OnScriptInput("hint19", () => {
    setText("Responsibility", hint_text1);
});
Instance.OnScriptInput("hint20", () => {
    setText("This is a good place to take a rest.", hint_text1);
});
Instance.OnScriptInput("hint21", () => {
    setText("I have to escape this place first", hint_text1);
});
Instance.OnScriptInput("hint22", () => {
    setText("My guess is its hidden at that tower", hint_text1);
});
Instance.OnScriptInput("hint23", () => {
    setText("So it really has started.", hint_text1);
});
...
Instance.OnScriptInput("ItemTick", (data) => {
    ...
    if ((item_name === "moly_weapon" && moly_lock == true) ||
        (item_name === "z_freeze_weapon" && freeze_lock == true)) {
        return;
    }
...
Instance.OnScriptInput("StartMissile", ({ caller }) => {
    const name = caller.GetEntityName();
    const suffix = name.match(/\d+$/)?.[0];
    EntFireTarget(caller, "SetSpeed", 100);
    EntFireTarget(caller, "Open");
    EntFireTarget(caller, "SetSpeed", 200, 3.5);
    EntFire("missile_prop" + suffix, "TurnOn");
    EntFire("missile_partic" + suffix, "Start");
});
Instance.OnScriptInput("ResetMissile", ({ caller }) => {
    const name = caller.GetEntityName();
    const suffix = name.match(/\d+$/)?.[0];
    EntFireTarget(caller, "Close");
    EntFire("missile_prop" + suffix, "TurnOff");
    EntFire("missile_partic" + suffix, "Stop");
});
Instance.OnScriptInput("EnableFogDark", (data) => {
    const fog = data.caller;
    EntFireTarget(fog, "SetFogEndDistance", 50000);
    EntFireTarget(fog, "Enable");
    let delay = 0;
    for (let end_dist = 50000; end_dist > 2000; end_dist -= 1000) {
        EntFireTarget(fog, "SetFogEndDistance", end_dist, delay);
        delay += 0.2;
    }
});
Instance.OnScriptInput("TeleportBoss", ({ activator }) => {
    const landmark = Instance.FindEntityByName("elevator_landmark");
    const vec = Vector3Utils.subtract(activator.GetAbsOrigin(), landmark.GetAbsOrigin());
    const velocity = activator.GetAbsVelocity();
    activator.Teleport({ position: Vector3Utils.add(boss_tp_dest, vec), velocity: new Vec3(velocity.x, velocity.y, 1000) });
...
Instance.OnScriptInput("BlastHurt", async (data) => {
    ...
    if (!(hurt_players.includes(activator))) {
        ...
        removeObject(hurt_players, activator);
    }
});
Instance.OnScriptInput("LockMoly", () => {
    moly_lock = true;
});
Instance.OnScriptInput("UnlockMoly", () => {
    moly_lock = false;
});
Instance.OnScriptInput("LockFreeze", () => {
    freeze_lock = true;
});
Instance.OnScriptInput("UnlockFreeze", () => {
    freeze_lock = false;
});
Instance.OnScriptInput("Ammo", (data) => {
    const activator = data.activator;
...
function setText(text, options) {
    ...
    for (const textPanelId of textPanelIds) {
        getHudLayout().SetHasClass(textPanelId, FADED_OUT_CLASS, true);
        getHudLayout().SetHasClass(textPanelId, OVERKILL_DIMMED_CLASS, false);
    }
...
function getChannelTextPanelIds(channel) {
    const { text, accent } = getChannelPanelIds(channel);
    return accent === undefined ? [text] : [text, accent];
}
...
function setHudText(text, channel = 1) {
    ...
    const hud = getHudLayout();
    hud.SetDialogVariableString(panelIds.text, TEXT_VARIABLE_NAME, text);
    if (panelIds.accent !== undefined) {
        hud.SetDialogVariableString(panelIds.accent, ACCENT_VARIABLE_NAME, '');
    }
}
function setHudTextAccent(text, channel) {
    const panelId = getChannelPanelIds(channel).accent;
    if (panelId === undefined) {
        throw new RangeError(`HUD text channel ${channel} has no accent label.`);
    }
    getHudLayout().SetDialogVariableString(panelId, ACCENT_VARIABLE_NAME, text);
}
...
function setHudTextSize(fontSize, channel = 1) {
    const hud = getHudLayout();
    const panelId = getChannelPanelIds(channel).text;
    const panelIds = getChannelTextPanelIds(channel);
    const state = channelStates[channel];
...
export { clearAllText, clearText, clearTextPlayer, fadeHudTextIn, fadeHudTextOut, setHudText, setHudTextColor, setHudTextFadeDuration, setHudTextPosition, setHudTextSize, setText, setTextPlayer, showKillThemAll, showOverkill };
