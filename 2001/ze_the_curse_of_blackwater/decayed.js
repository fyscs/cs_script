...
let decayedBox = null;                // decayed_ct实体
...
Instance.OnScriptInput("start", (inputData) => {
    ...
    if (inputData.caller) {
        ...
        if (suffixMatch && suffixMatch[1]) {
            ...
            // 查找对应的decayed_ct实体
            const boxName = `decayed_ct_${relaySuffix}`;
            decayedBox = Instance.FindEntityByName(boxName);
            
            if (decayedBox && decayedBox.IsValid()) {
                // 重置状态
...
function calculateInitialHealth() {
    ...
    // 3. 获取decayed_base_health实体角度总和
    let baseHealthAngleSum = 0;
    const baseHealthEntity = Instance.FindEntityByName("decayed_base_health");
    ...
    // 5. 设置到对应的血量计数器
    const hpCounterName = `decayed_hp_counter_${relaySuffix}`;
...
Instance.SetThink(() => {
    ...
    if (!isChecking || !decayedBox || !decayedBox.IsValid()) return;
    
    const boxPos = decayedBox.GetAbsOrigin();
    const boxAngles = decayedBox.GetAbsAngles();
    ...
    if (isTracking) {
        ...
        if (targetPlayer && targetPlayer.IsValid() && targetPlayer.IsAlive() && 
            ...
            // 计算新速度
            const currentVelocity = decayedBox.GetAbsVelocity();
            let newVelocity = {
