...
let creepygirlBox = null;                // creepygirl_ct实体
...
Instance.OnScriptInput("start", (inputData) => {
    ...
    if (inputData.caller) {
        ...
        if (suffixMatch && suffixMatch[1]) {
            ...
            // 查找对应的creepygirl_ct实体
            const boxName = `creepygirl_ct_${relaySuffix}`;
            creepygirlBox = Instance.FindEntityByName(boxName);
            
            if (creepygirlBox && creepygirlBox.IsValid()) {
                // 重置状态
...
function calculateInitialHealth() {
    ...
    // 3. 获取creepygirl_base_health实体角度总和
    let baseHealthAngleSum = 0;
    const baseHealthEntity = Instance.FindEntityByName("creepygirl_base_health");
    ...
    // 5. 设置到对应的血量计数器
    const hpCounterName = `creepygirl_hp_counter_${relaySuffix}`;
...
Instance.SetThink(() => {
    ...
    if (!isChecking || !creepygirlBox || !creepygirlBox.IsValid()) return;
    
    const boxPos = creepygirlBox.GetAbsOrigin();
    const boxAngles = creepygirlBox.GetAbsAngles();
    ...
    if (isTracking) {
        ...
        if (targetPlayer && targetPlayer.IsValid() && targetPlayer.IsAlive() && 
            ...
            // 计算新速度
            const currentVelocity = creepygirlBox.GetAbsVelocity();
            let newVelocity = {
