...
const SCRIPT = "Fling_Script"

const DISTANCE = 768;
const FORCE_MOD  = 1.00;
const UP_SPEED = 400;

Instance.OnScriptInput("FlingTheHumans", ({ caller, activator }) => {
    if(caller?.IsValid && caller.GetClassName() == "func_button" && caller.GetEntityName().includes("item_button_2"))
    {
        let spos = caller.GetAbsOrigin();
        let humans = getHumansInRadius(spos, DISTANCE);
        const visibleHumans = humans.filter(player => {
            return InSight(spos, GVO(player.GetAbsOrigin(),0,0,48), caller, true);
        });

        Instance.Msg(`Found ${humans.length} humans in radius, ${visibleHumans.length} are visible.`);

        for(let i = 0; i < visibleHumans.length; i++)
        {
            let player = visibleHumans[i];
            let spos = caller.GetAbsOrigin();
            let tpos = player.GetAbsOrigin();
            let vec = { x: spos.x - tpos.x, y: spos.y - tpos.y, z: spos.z - tpos.z };
            let dist = GetDistance(spos,GVO(tpos,0,0,48));
            vec = Vector3Utils.normalize(vec);
            player.Teleport({ velocity: { x: vec.x*(((dist)-DISTANCE)*FORCE_MOD), y: vec.y*(((dist)-DISTANCE)*FORCE_MOD), z: UP_SPEED } });
        }
    }
});
...
function InSight(startl, targetl, ignoreEnt, ignorePlayers)
{
    ...
    Instance.DebugLine({ start: startl, end: trace.end, color: trace.fraction < 1.00 ? {r:255, g:0, b:0} : {r:0, g:255, b:0}, duration: 3.00 });
