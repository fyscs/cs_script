...
class SpawnData {
    constructor(templateName, origin, angle, originOffset = { x: 0, y: 0, z: 0 }, angleOffset = { pitch: 0, yaw: 0, roll: 0 }) {
    ...
    Spawn() {
        const template = Instance.FindEntityByName(this.templateName)
            || (this.templateName === "temp" ? Instance.FindEntityByName("@temp") : undefined);
        ...
        const spawnAng = {
            pitch: this.angle.pitch + GetRandomInt(-this.angleOffset.pitch, this.angleOffset.pitch),
            yaw: this.angle.yaw + GetRandomInt(-this.angleOffset.yaw, this.angleOffset.yaw),
            roll: this.angle.roll + GetRandomInt(-this.angleOffset.roll, this.angleOffset.roll)
        };
...
function GetAliveCTPawns() {
    ...
    for (let slot = 0; slot < 64; slot++) {
        ...
        if (controller && controller.IsValid() && controller.GetTeamNumber() === CT_TEAM) {
    ...
    try {
        ...
        for (let slot = 0; slot < 64; slot++) {
            ...
            if (controller && controller.IsValid() && controller.GetTeamNumber() === CT_TEAM) {
                const pawn = controller.GetPlayerPawn();
                if (pawn && pawn.IsValid() && pawn.IsAlive()) {
                    ctPlayers.push({ controller, pawn, slot });
                }
            }
        }
        ...
        const playerName = randomCT.controller.GetPlayerName();
...
function GetPlayerSlotKey(inputData) {
    ...
    if (entity.GetClassName() === "player") {
        ...
        return controller ? controller.GetPlayerSlot() : null;
    }

    if (entity.GetClassName() === "player_controller") {
...
function ProcessCooldowns() {
    ...
    if (hasNpcMovement || hasLolTasks || hasZM || hasMilkman || hasShopinTasks) {
        Instance.SetNextThink(0.02);
...
function playSecretSoundForActivator(activator) {
    ...
    const controller = typeof pawn.GetPlayerController === "function"
...
Instance.OnScriptInput("tpteam", (inputData) => {
    try {
        ...
        for (const p of players) {
            ...
            const slot = controller ? controller.GetPlayerSlot() : -1;
