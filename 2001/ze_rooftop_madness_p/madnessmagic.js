...
let rainbow_road_saver = true;
let rainbow_road = false;
//MAP THINK
Instance.SetThink(() => {
...
Instance.SetNextThink(Instance.GetGameTime());
function FunctionDelay(func, delay, ...args) {
    setTimeout(() => func(...args), delay * 1000);
}
...
Instance.OnRoundEnd((stuff) => {
    if (stuff.winningTeam == 3 && rainbow_road) {
        rainbow_road_saver = false;
    }
});
const music_backup = ["music_1", "music_2", "music_3", "music_4", "music_5", "music_6", "music_7", "music_8"];
let music = [];
Instance.OnRoundStart(() => {
    clearTasks();
    rainbow_road = false;
    FunctionDelay(dj, 7);
    if (rainbow_road_saver) {
        Instance.EntFireAtName({ name: "tp_rainbow", input: "Enable" });
    }
});
function dj() {
    if (rainbow_road) {
    ...
    if (music.length == 0) {
        music = music_backup.slice();
    }
    const rng = GetRandomInt(0, music.length - 1);
    music.slice(rng, 1);
    const ent = Instance.FindEntityByName(music[rng]);
    const id = Instance.ConnectOutput(ent, "OnSoundFinished", () => {
        if (!rainbow_road) {
            dj();
        }
        Instance.DisconnectOutput(id);
    });
    Instance.EntFireAtTarget({ target: ent, input: "StartSound", delay: 0 });
}
...
Instance.OnScriptInput("input_rainbow_road", () => {
    rainbow_road = true;
});
Instance.OnScriptInput("input_healthshot", (stuff) => {
    const time = Instance.GetGameTime() + 3;
    const ent = Instance.FindEntityByName("helper_heal");
    const interval = setInterval(() => {
        if (Instance.GetGameTime() > time) {
            ...
            for (const player of players) {
                if (player?.GetTeamNumber() == 3 && Vector3Utils.distance(player.GetAbsOrigin(), ent.GetAbsOrigin()) < 256) {
