...
function StartBoss() {
    ...
    Instance.SetThink(CheckHealth, Instance.GetGameTime() + TICKRATE_B);
}
...
function CheckHealth() {
    ...
    BuildHud();
    Instance.SetNextThink(Instance.GetGameTime() + TICKRATE_B);
}
...
function BuildHud() {
    ...
    if (BOSS_PERCENT_C) {
        Instance.EntFireAtName(BOSS_PERCENT_C, "InValue", `${PERCENT_HP}`);
    }
    ...
    Instance.EntFireAtName(BOSS_HUD_ENT, "SetMessage", BOSS_HUD_TEXT);
}
...
Instance.OnScriptInput("ItemDamage", (inputData) => {
    if (!IS_BOSS_FIGHT) return;
    
    const arg = inputData.value?.toString() || "item,0";
    const arg_rs = arg.replace(/\s+/g, '');
    const arr = arg_rs.split(",");
    
    const itemName = arr[0] || "item";
