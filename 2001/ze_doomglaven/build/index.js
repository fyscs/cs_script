...
const announce = (message, delay) => Instance.EntFireAtName({ name: "server", input: "Command", value: `say *${message.toUpperCase()}*`, delay });
...
const spawnChest = (location, position, type) => Instance.EntFireAtName({ name: `spawnable_chest-${type}_maker`, input: "ForceSpawnAtEntityOrigin", value: `spawner_chest_${location}${position}` });
const spawnChestsForLocation = (location) => {
    ...
    chestTypes.forEach(chestType => {
        ...
        spawnPositions.forEach(position => {
            spawnChest(location, position, chestType);
        });
    });
};
...
const spawnAllGrimoires = () => {
    ...
    Instance.EntFireAtName({ name: "spell_grimoire-gnashing_maker", input: "ForceSpawnAtEntityOrigin", value: `spawner_grimoire${grimoirePositions[0]}` });
    Instance.EntFireAtName({ name: "spell_grimoire-gasping_maker", input: "ForceSpawnAtEntityOrigin", value: `spawner_grimoire${grimoirePositions[1]}` });
    Instance.EntFireAtName({ name: "spell_grimoire-gyration_maker", input: "ForceSpawnAtEntityOrigin", value: `spawner_grimoire${grimoirePositions[2]}` });
    Instance.EntFireAtName({ name: "spell_grimoire-gluttony_maker", input: "ForceSpawnAtEntityOrigin", value: `spawner_grimoire${grimoirePositions[3]}` });
};
...
const placeGrimoire = (grimoireName) => {
    placedGrimoires++;
    Instance.EntFireAtName({ name: `spell_grimoire-${grimoireName}_book`, input: "Kill" });
    Instance.EntFireAtName({ name: `spell_grimoire-${grimoireName}_trigger`, input: "Kill" });
    Instance.EntFireAtName({ name: `ritual_placedbook${placedGrimoires}`, input: "Enable" });
    Instance.EntFireAtName({ name: `ritual_lightning_timer${placedGrimoires}`, input: "Enable" });
    Instance.EntFireAtName({ name: "ritual_book_sound", input: "StartSound" });
    ...
    if (placedGrimoires === 3) {
        Instance.EntFireAtName({ name: "ritual_start_relay", input: "Trigger" });
    }
...
const triggerRitualLightning = () => {
    ...
    Instance.EntFireAtName({ name: `library_window${windowNumber}_particle`, input: "Start" });
    Instance.EntFireAtName({ name: `library_window${windowNumber}_particle`, input: "Stop", delay: 2 });
    Instance.EntFireAtName({ name: `library_window${windowNumber}_particle2`, input: "Start" });
    Instance.EntFireAtName({ name: `library_window${windowNumber}_particle2`, input: "Stop", delay: 2 });
    Instance.EntFireAtName({ name: `library_window${windowNumber}_breakable`, input: "Break" });
    Instance.EntFireAtName({ name: "ritual_lightning_sound", input: "StartSound" });
    Instance.EntFireAtName({ name: "sound_lightning_distant", input: "StartSound" });
};
...
Instance.OnScriptInput("triggerOutsideLightning", () => {
    ...
    Instance.EntFireAtName({ name: `library_random-lightning${lightningNumber}`, input: "ForceSpawn" });
});
Instance.OnScriptInput("spawnRandomSpawnables", () => {
    spawnAllChests();
    spawnAllGrimoires();
});
const boostAmount = 300;
Instance.OnScriptInput("triggerThunderItem", ({ activator }) => {
    if (!activator) {
        return;
    }
    const spawner = Instance.FindEntitiesByName("effect_thunder_template")[0];
    if (!spawner || !(spawner instanceof PointTemplate)) {
        return;
    }
    const currentOrigin = new Vec3(activator.GetAbsOrigin());
    const currentDirection = new Euler(activator.GetEyeAngles());
    spawner.ForceSpawn(currentOrigin.add(new Vec3(0, 0, 96)), currentDirection.normal);
    const backwardsVelocity = new Vec3({
        x: -currentDirection.forward.x * boostAmount,
        y: -currentDirection.forward.y * boostAmount,
        z: -currentDirection.forward.z * boostAmount
    });
