...
class NPC {
  constructor(_suffix, _model, _physbox) {
    ...
    this.NPC_RETARGET_TIME = 5.0
    this.NPC_CURRENT_TAR_TIME = 0.0

    this.NPC_SIGHT_CHECK_TIME = 0.5
    this.NPC_CURRENT_SIGHT_TIME = 0.0
  ...
  FindPlayer() {
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      if (
        ...
        VectorDistance(player.GetAbsOrigin(), this.npc_model.GetAbsOrigin()) <=
          this.NPC_TARGET_DIST &&
        TargetInSight(
          player.GetAbsOrigin(),
          this.npc_model.GetAbsOrigin(),
          this.physbox,
          this.NPC_Z_OFFSET,
        )
      ) {
    ...
    if (hlist.length > 0) {
      ...
      this.physbox.Move({
        velocity: { x: 0, y: 0, z: this.physbox.GetAbsVelocity().z },
...
class NPC_Default extends NPC {
  ...
  onTick(delta) {
    ...
    if (
      !this.NPC_TARGET?.IsValid() ||
      !this.NPC_TARGET?.IsAlive() ||
      this.NPC_TARGET?.GetTeamNumber() !== 3 ||
      this.NPC_CURRENT_TAR_TIME >= this.NPC_RETARGET_TIME
    ) {
    ...
    const turnSpeed = Math.min(
      absYawDiff * 15 * this.npc_turn_speed,
      this.MAX_TURN_SPEED,
    )
    ...
    if (this.lastPos) {
      ...
      const minMovement = Math.max(expectedMovement * 0.2, 0.5)
    ...
    const finalVelocity = isStuck
      ? { x: 0, y: 0, z: this.physbox.GetAbsVelocity().z }
      : velocity

    this.physbox.Move({
      angles: { pitch: 0, yaw: ang.yaw, roll: 0 },
      velocity: finalVelocity,
      angularVelocity: inFront ? { x: 0, y: 0, z: 0 } : angularVelocity,
    })
  }
...
Instance.OnScriptInput('SetNpcMutant', ({ caller, activator }) => {
  if (caller?.IsValid()) {
    ...
    if (!NPC_MAP.has(caller_name)) {
      ...
      npc_instance.SetNpcSpeed(1.2)
      npc_instance.SetNpcRetargetTime(5.0)
      npc_instance.SetNpcTurnSpeed(1.0)
...
function TargetInSight(player_pos, boss_pos, ignore_entity, z_offset) {
  ...
  if (
    hitClass === 'func_button' ||
    hitClass?.includes('weapon_') ||
    hitClass === 'func_physbox'
  ) {
    visible = true
  }
