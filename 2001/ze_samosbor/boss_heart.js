...
const InPuts = [
  [
    'Map_Boss_TheHeart_Start',
    'OnTrigger',
    'Map_Boss_TheHeart_GetDamage,Map_Boss_Health_Script,Map_Boss_Health_Hud,[The$Heart]$[2/2],100,1',
    StartBoss,
    3.0,
  ],
  [
    'Boss_Phase_Case',
    'OnCase01',
    'Map_Boss_TheHeart_GetDamage,Map_Boss_Health_Script,Map_Boss_Health_Hud,[The$Heart]$[1/2],50,1',
    StartBoss,
    0.1,
  ],
  ['Map_Boss_TheHeart_Start', 'OnTrigger', '0,450', AddHealth, 2.9],
  ['Filter_Team_Human_BossAdd_P2', 'OnPass', '1,580', AddHealth, 0.0],
  ['Map_Boss_TheHeart_Health_Branch', 'OnTrue', '-5', ChangeHealthIt, 0.0],
  [
    'Map_Boss_Arena_Bottom_SubtractHealth',
    'OnStartTouch',
    '70',
    ChangeHealthIt,
    0.0,
...
Instance.OnScriptInput('SubtractHealth', ({ activator }) => {
  if (!IS_BOSS_FIGHT) {
    return
  }
...
function ChangeHealthIt(arg, activator) {
  if (!IS_BOSS_FIGHT) {
    return
  }
  if (BOSS_HEALTH >= 0) {
...
function GrenadeDamage(arg, activator) {
  if (!IS_BOSS_FIGHT) {
    return
  }
  if (BOSS_HEALTH >= 0) {
  ...
  GRENADE_DAMAGE_TICK = 2.0
}

function ItemDamage(arg, activator) {
  if (!IS_BOSS_FIGHT) {
    return
  }
  let arg_s = arg
