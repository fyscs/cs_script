...
class BeanNPC {
    ...
    checkAwaken() {
        ...
        for (const player of players) {
            ...
            if (physboxOrigin.subtract(playerHead).length() <= this.aggroRange && this.hasLineOfSight(player)) {
                ...
                if (this.model?.IsValid()) Instance.EntFireAtTarget({ target: this.model, input: "SetAnimationLooping", value: "npc001_bean_anim_jump" });
    ...
    onDeath() {
        ...
        if (this.physbox?.IsValid()) {
            Instance.EntFireAtTarget({ target: this.physbox, input: "Kill" });
        }
...
class BeanNPCManager {
    ...
    static connect(relayEntity) {
        ...
        const npc = new BeanNPC(relayEntity);
...
Instance.OnActivate(() => {
    BeanNPCManager.lastTickTime = Instance.GetGameTime();
});
...
Instance.OnScriptReload({
    before: () => {
        const npcData = [];
        for (const [name, npc] of BeanNPCManager.instances.entries()) {
            npcData.push({ name, awake: npc.awake, dead: npc.dead, target: npc.target?.IsValid() ? npc.target.GetEntityName() : null });
        }
        return { npcs: npcData };
    },
    after: () => {}
});
Instance.OnScriptInput("connect_bean", (event) => {
    BeanNPCManager.connect(event.caller);
});
