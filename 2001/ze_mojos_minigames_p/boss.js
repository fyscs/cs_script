...
function spawnColumns(n) {
    let i = 0;
    const templ = Instance.FindEntityByName("templ_boss_column");
    while (n > 1) {
        const dir = getRandomInt(0, 3);
        let origin = new Vec3(_x + getRandomFloat(-1024, 1024), _y + getRandomFloat(-1024, 1024), templ.GetAbsOrigin().z);
        let angle = templ.GetAbsAngles();
        switch (dir) {
            case 0:
                origin.x = _x - 1536;
                break;
        ...
        Instance.EntFireAtTarget({ target: templ, input: "KeyValue", value: "origin " + origin.x + " " + origin.y + " " + origin.z, delay: Math.sqrt(i) * 2 + 1 });
        Instance.EntFireAtTarget({ target: templ, input: "KeyValue", value: "angles " + angle.pitch + " " + angle.yaw + " " + angle.roll, delay: Math.sqrt(i) * 2 + 1.02 });
        Instance.EntFireAtTarget({ target: templ, input: "ForceSpawn", delay: Math.sqrt(i) * 2 + 1.05 });
        i++;
        n--;
    }
}
    ...
    while (n > 1) {
        ...
        switch (dir) {
            case 0:
                ...
                maker_pos.z = z;
                maker.Teleport({ position: maker_pos });
                break;
            case 1:
        ...
        Instance.EntFireAtTarget({ target: maker, input: "ForceSpawn", delay: Math.sqrt(i) * 3 + 1 });
        i++;
        n--;
    }
}
