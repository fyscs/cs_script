...
let plats_left = 0;
let crumble = false;
Instance.OnScriptInput("SpawnPlatform", () => {
    const templ = Instance.FindEntityByName("templ_platform");
    for (let i = 0; i < side; i++) {
        plats[i] = [];
        for (let j = 0; j < side; j++) {
            if (Vector3Utils.distance(new Vec3(i, j, 0), new Vec3(4.5, 4.5, 0)) > 7)
                continue;
            const pos = new Vec3(x + i * l, y + j * l, z + h / 2);
            const plat = templ.ForceSpawn(pos, new Euler(0, 0, 0));
            plats[i][j] = plat[0];
            plats_left++;
        }
    }
    Instance.EntFireAtName({ name: "script_crumble", input: "RunScriptInput", value: "Crumble", delay: 5 });
    crumble = true;
});
Instance.OnScriptInput("BackIn", () => {
    if (plats_left > 36) {
        Instance.EntFireAtName({ name: "hudhint", input: "SetMessage", value: plats_left-- - 36 + " platforms left to win" });
        Instance.EntFireAtName({ name: "show_hud", input: "CountPlayersInZone", delay: 0.02 });
    }
});
let delay = 1.5;
let rate = 1.7;
Instance.OnScriptInput("Crumble", () => {
    if (!crumble)
        return;
    if (plats_left <= 80)
        rate = 1.4;
    ...
    if (plats_left === 51) {
        Instance.ServerCommand("say * * * 15 platforms more to win! * * *");
    }
    if (plats_left <= 36) {
        delay = 1.5;
        rate = 1.7;
    ...
    while (cell == undefined) {
        const i = getRandomInt(0, side - 1);
        const j = getRandomInt(0, side - 1);
        cell = plats[i][j];
        if (cell != undefined && cell.IsValid()) {
            Instance.EntFireAtTarget({ target: cell, input: "Color", value: "255 0 0" });
            Instance.EntFireAtTarget({ target: cell, input: "Break", delay: delay });
            Instance.EntFireAtName({ name: "script_crumble", input: "RunScriptInput", value: "Crumble", delay: rate });
        }
        else {
            cell = undefined;
        }
