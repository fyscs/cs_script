...
class RionBossController {
  ...
  initialize() {
    ...
    if (!this.physbox || !this.physbox.IsValid()) {
      ...
      if (this._init_attempts < this._max_init_attempts) {
        ...
        Instance.Msg(
          '[RionBoss] ❌ 未找到物理实体 ' + boss_config.physbox_name + '！\n',
        )
        return false
      }
    }
  ...
  checkWallStuck(selfPos) {
    ...
    if (this.last_position) {
      ...
      if (this.stuck_timer >= this.stuck_check_time && !this.is_stuck) {
        ...
        const dirX =
          this.stuck_push_direction.x * cosA -
          this.stuck_push_direction.y * sinA
        const dirY =
          this.stuck_push_direction.x * sinA +
          this.stuck_push_direction.y * cosA
  ...
  applyReversePush() {
    ...
    const newVel = {
      x:
        currentVel.x +
        this.stuck_push_direction.x * currentForce * this.tickrate,
      y:
        currentVel.y +
        this.stuck_push_direction.y * currentForce * this.tickrate,
      z: currentVel.z,
    }

    const maxPushSpeed = 1200
    const speed = Math.sqrt(
      newVel.x * newVel.x + newVel.y * newVel.y + newVel.z * newVel.z,
    )
