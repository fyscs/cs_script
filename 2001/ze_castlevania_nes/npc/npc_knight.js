...
function createNPC(suffix, ctEntity) {
    const hitboxName = "npc_knight_hitbox_" + suffix;
    const hudName = "npc_knight_hud_" + suffix;
    ...
    return {
        ...
        Fireuser3Time: 0,
        trackingEnableTime: 0,      // 追踪启用时间（延迟1秒）
        fireuser3Sent: false,       // Fireuser3 是否已发送
        targetList: [],
        currentTarget: null,
...
function checkVisionForNPC(npc) {
    ...
    for (const player of players) {
        ...
        if (traceResult.didHit && traceResult.hitEntity === player) {
            ...
            const now = Instance.GetGameTime();
            npc.Fireuser3Time = now + 0.0;           // Fireuser3 延迟 3 秒
            npc.trackingEnableTime = now + 3.0;      // 追踪启用延迟 1 秒
            npc.fireuser3Sent = false;
            return;
        }
    }
}
...
function processPendingFire(npc) {
    ...
    // 启用追踪（延迟1秒）
    if (!npc.isTracking && now >= npc.trackingEnableTime) {
        npc.isTracking = true;
        updateTargetList(npc);
        selectNewTarget(npc);
        npc.nextTargetChangeTime = now + CONFIG.TARGET_REFRESH_INTERVAL;
    }
    if (!npc.isTracking) allDone = false;

    // 发送 Fireuser3（延迟3秒）
    if (!npc.fireuser3Sent && now >= npc.Fireuser3Time) {
        Instance.EntFireAtTarget({
            target: npc.ct,
            input: "Fireuser3",
        });
        npc.fireuser3Sent = true;
    }
    if (!npc.fireuser3Sent) allDone = false;
