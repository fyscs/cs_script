...
Instance.OnRoundStart(() => {
    ...
    savedVel = 0;
    savedAcc = 0;
    Instance.Msg("Reset Eye2 Variables");
});
Instance.OnScriptInput("FindEye2", () => {
    eye2 = Instance.FindEntityByName("EyeBoss2");
});
...
function Retarget() {
    ...
    for (const currentPlayer of playerArray) {
        if (currentPlayer.GetTeamNumber() == bossTeamTarget && currentPlayer.GetHealth() > 0) {
            let length = Vector3Utils.distance(eye2.GetAbsOrigin(), currentPlayer.GetAbsOrigin());
...
function FakePhysicsMovement() {
    ...
    if (!eye2)
        return;
    let cnt = eye2.GetAbsOrigin();
    // Make the boss face the target
    eye2.Teleport(null, Vector3Utils.vectorAngles(vel), null);
    ...
    if (collPoint) {
        ...
        vel = vrefl(vel, norm);
        Instance.EntFireAtName("EyeBoss2", "FireUser1", "", 0.0);
        // Move as far as we can
        eye2.Teleport(Vector3Utils.add(cnt, Vector3Utils.scale(norm, bossRadius)), null, null);
    }
    else {
        eye2.Teleport(Vector3Utils.add(cnt, vel), null, null);
    }
}
