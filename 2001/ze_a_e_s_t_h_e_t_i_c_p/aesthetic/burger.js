...
Instance.Msg("Loaded Burger Trigger Script");
var king;
Instance.OnRoundStart(() => {
    // Reset variables and find entities
    king = Instance.FindEntityByName("Burger_Trigger");
});
Instance.OnScriptInput("CheckBurger", () => {
    // Sanity check
    if (!king) {
        return;
    }
    // Check for burger in radius
    let burgerArray = Instance.FindEntitiesByName("Burger_Fire*");
    for (const burger of burgerArray) {
        if (InDistance(burger.GetAbsOrigin(), king.GetAbsOrigin())) {
            Instance.EntFireAtName("Burger_Fed", "Add", "1", 0.0);
            Instance.EntFireAtTarget(burger, "FireUser1", "", 0.0);
        }
    }
});
function InDistance(burg, king) {
    if (Vector3Utils.distance(burg, king) < 48)
        return true;
    else
        return false;
}
