...
Instance.Msg("Loaded Chess Script");
...
Instance.OnRoundStart(() => {
    ...
    purpleX = 4;
    purpleY = 1;
    greenStart = Instance.FindEntityByName("GreenStart");
    greenEnd = Instance.FindEntityByName("GreenEnd");
    purpleStart = Instance.FindEntityByName("PurpleStart");
    purpleEnd = Instance.FindEntityByName("PurpleEnd");
});
function Check(x, y) { return -1 < x && x < 8 && -1 < y && y < 8 && !((x == greenX && y == greenY) || (x == purpleX && y == purpleY)); }
function createArgumentScriptInput(variants, callback) {
    for (const variant of variants) {
        const match = variant.match(/\w*\((.*)\)$/);
        if (match) {
            const args = match[1];
            Instance.OnScriptInput(variant, (context) => {
                callback(context, ...args.split(',').map((arg) => arg.trim()));
            });
        }
    }
}
createArgumentScriptInput(["MoveGreen(-1, -1)", "MoveGreen(-1, 1)", "MoveGreen(1, -1)", "MoveGreen(1, 1)", "MoveGreen(-1, 0)", "MoveGreen(1, 0)", "MoveGreen(0, -1)", "MoveGreen(0, 1)"], (context, dx, dy) => {
    if (Check(greenX + parseInt(dx), greenY + parseInt(dy))) {
        UpdateGreen(parseInt(dx), parseInt(dy));
        UpdateGreenEnd(parseInt(dx), parseInt(dy));
    }
});
createArgumentScriptInput(["MovePurple(-1, -1)", "MovePurple(-1, 1)", "MovePurple(1, -1)", "MovePurple(1, 1)", "MovePurple(-1, 0)", "MovePurple(1, 0)", "MovePurple(0, -1)", "MovePurple(0, 1)"], (context, dx, dy) => {
    if (Check(purpleX + parseInt(dx), purpleY + parseInt(dy))) {
        UpdatePurple(parseInt(dx), parseInt(dy));
        UpdatePurpleEnd(parseInt(dx), parseInt(dy));
    }
});
function UpdateGreen(dx, dy) { greenX += dx; greenY += dy; }
function UpdateGreenEnd(dx, dy) {
    if (!greenEnd)
        return;
    let currentPos = greenEnd.GetAbsOrigin();
    let moveVector = new Vec3(dx * CELL_SIZE, dy * CELL_SIZE, 0.0);
    greenEnd.Teleport(Vector3Utils.add(currentPos, moveVector), null, null);
}
function UpdatePurple(dx, dy) { purpleX += dx; purpleY += dy; }
function UpdatePurpleEnd(dx, dy) {
    if (!purpleEnd)
        return;
    let currentPos = purpleEnd.GetAbsOrigin();
    let moveVector = new Vec3(dx * CELL_SIZE, dy * CELL_SIZE, 0.0);
    purpleEnd.Teleport(Vector3Utils.add(currentPos, moveVector), null, null);
}
Instance.OnScriptInput("MoveStartToEndGreen", () => {
    if (!greenStart || !greenEnd)
        return;
    greenStart.Teleport(greenEnd.GetAbsOrigin(), null, null);
});
Instance.OnScriptInput("MoveStartToEndPurple", () => {
    if (!purpleStart || !purpleEnd)
        return;
    purpleStart.Teleport(purpleEnd.GetAbsOrigin(), null, null);
});
Instance.OnScriptInput("CheckInLove", () => {
    let dx = greenX - purpleX;
    let dy = greenY - purpleY;
    if (dx < 0)
        dx *= -1;
    if (dy < 0)
        dy *= -1;
...
function RunLoveSequence() {
    Instance.EntFireAtName("TotalTrigger_relay", "Trigger", "", 0.0);
}
