...
const npc_name = "npc_girlcat_hbox*";
const NPC_MAKER = "girlcat_add_temp";
const boss_script = "girlcat_npc_script";
let NPC_ENT = null;
let NPC_ENTS_ASPAWN = null;
let IGNORE_BOSS = [];

const BOSS_ENTITIES_MAP = {
    "npc_girlcat_hbox": "SetBossPhysBoxEnt",
    "npc_girlcat_phys": "SetBossTrainEnt",
    "npc_girlcat_path_01": "SetBossFirstPathEnt",
    "npc_girlcat_path_02": "SetBossSecPathEnt",
    "npc_girlcat_model": "SetBossModelEnt",
};
...
class BossMove {
    constructor(suffix) {
        ...
        this.Refire_Time_Base = THINK_INTERVAL;
        this.Time_N = 0.00;
...
Instance.OnScriptInput("Start", ({ caller }) => {
...
Instance.OnScriptInput("SpawnNpc", ({ caller }) => {
    NPC_ENT = Instance.FindEntityByName(NPC_MAKER);
    if (NPC_ENT?.IsValid() && caller?.IsValid()) {
        let spawnPos = caller.GetAbsOrigin();
        NPC_ENTS_ASPAWN = NPC_ENT.ForceSpawn(spawnPos);
        ...
        Instance.Msg("SpawnNpc failed: Invalid caller or NPC_MAKER.");
    }
});
...
function IsValidEntityTeam(ent, t) {
    return ent?.IsValid() && ent?.GetHealth() > 0 && ent?.GetTeamNumber() == t;
}
...
Instance.OnScriptInput("DisableMove", ({ caller }) => {
    const ent_name = caller?.GetEntityName();
    const suffix = ent_name.match(/_\d+$/);
    const bossInstance = BossInstancesMap.get(suffix?.[0]);
    if (bossInstance) {
...
Instance.OnScriptInput("EnableMove", ({ caller }) => {
    const ent_name = caller?.GetEntityName();
    const suffix = ent_name.match(/_\d+$/);
    const bossInstance = BossInstancesMap.get(suffix?.[0]);
    if (bossInstance) {
...
Instance.OnScriptInput("BossKill", ({ caller }) => {
    const ent_name = caller?.GetEntityName();
    const suffix = ent_name.match(/_\d+$/);
    const bossInstance = BossInstancesMap.get(suffix?.[0]);
    if (bossInstance) {
