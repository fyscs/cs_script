...
function ensureSchedulerTick() {
    if (schedulerQueued || tasks.length === 0)
        return;
...
Instance.OnRoundStart(() => {
    // wiping this var in case it persists along the knife throughout rounds
    for (const e of Instance.FindEntitiesByClass("weapon_*"))
        delete e.item;
    clearTasks();
});
const zmItemContexts = ["deathuser", "hasteuser", "curseuser"];
Instance.OnPlayerReset((event) => {
    // wiping contexts on respawning players to let zombie take another item after death
    // zombie items don't advance forward afaik and zombie would have to die on round start
    // is this possible? DUNNO!
    for (const context of zmItemContexts)
        Instance.EntFireAtTarget({ target: event.player, input: "RemoveContext", value: context });
});
Instance.OnScriptInput("InitItem_OnPickup", (event) => {
    const weapon = event.caller;
    // knife can only be picked up once anyway, no need for RunOnceOnly safeguard
    const owner = event.activator;
    // ... but one person can pick up multiple items in one round
    // @ts-ignore
    if (owner.itemIntervalIdx !== undefined) {
        // @ts-ignore
        clearInterval(owner.itemIntervalIdx);
    }
    const idx = setInterval(() => {
        if (!owner.IsValid() || !owner.IsAlive() || owner.GetTeamNumber() != Team.CT) {
            clearInterval(idx);
            return;
        }
        const usePressed = owner.IsInputPressed(CSInputs.USE);
        // @ts-ignore
        const wasUseHeld = owner.itemUseHeld;
        // @ts-ignore
        owner.itemUseHeld = usePressed;
        if (usePressed && !wasUseHeld) {
            // trying to make it so item owners can press map buttons without triggering item
            const traceEnd = new Euler(owner.GetEyeAngles()).forward.scale(80).add(owner.GetEyePosition());
            let ignoreEnts = Instance.FindEntitiesByClass("*");
            ignoreEnts = ignoreEnts.filter((el) => el.GetClassName() !== "func_button" && el.GetClassName() !== "worldent");
            const trace = Instance.TraceLine({
                start: owner.GetEyePosition(),
                end: traceEnd,
                ignorePlayers: true,
                ignoreEntity: ignoreEnts
            });
            if (trace.hitEntity && trace.hitEntity.GetClassName() === "func_button")
                return;
            Instance.EntFireAtName({
                name: weapon.item + "_relayfix",
                input: "Trigger",
                caller: owner,
                activator: owner
            });
    ...
    // @ts-ignore
    owner.itemIntervalIdx = idx;
});
Instance.OnScriptInput("InitItemButtonReplacement", (event) => {
    const weapon = event.caller;
    const nameSplit = weapon.GetEntityName().split("_");
    let item = nameSplit[0];
    if (nameSplit[1].endsWith('2'))
        item = item.concat('2');
    weapon.item = item;
});
Instance.OnScriptInput("STUPIDTIDALWAVEPUSH", (event) => {
    const trigger = event.caller;
    const triggerAngles = new Euler(trigger.GetAbsAngles());
    triggerAngles.pitch -= 10;
    const v = triggerAngles.forward.scale(-240); // I guess that trigger faces oppposite direction
    Instance.EntFireAtTarget({
        target: trigger,
        input: "AddOutput",
        value: `OnTouchingEachEntity>!activator>KeyValue>basevelocity ${v.x.toPrecision(2)} ${v.y.toPrecision(2)} ${v.z.toPrecision(2)}>0.0>-1`
    });
    Instance.EntFireAtTarget({
        target: trigger,
        input: "AddOutput",
        value: `OnStartTouch>!activator>KeyValue>basevelocity ${v.x.toPrecision(2)} ${v.y.toPrecision(2)} ${v.z.toPrecision(2)}>0.0>-1`
    });
});
