...
const boss_hp_add = 250;//boss动态血量,乘以人数后累加至血量中
const boss_script = "boss2_script";//boss脚本实体targetname配置
const boss_template_targetname = "boss2_template";//boss模板实体targetname配置
const boss_position_targetname = "boss2_spawn_point";//boss生成位置targetname配置
const boss_phys_targetname = "Stage3_Minion";//boss物理碰撞实体的targetname配置
const boss_hpbar_targetname = "boss_hp_text_2";//boss血条实体的targetname配置
const boss_model_targetname = "Stage3_Minion_Model";//boss模型的targetname配置
...
class BossMain {
    ...
    boss_death() {
        ...
        if (!boss_death_animation_boolean) {
            ...
            Instance.EntFireAtName({ name: "ShitFuckShitFuck", input: "Break" });
            Instance.EntFireAtName({ name: "L3_BossFightEnd", input: "add",paramater:"1" });
            return;
        }
        else {
            this.isActive = false;
