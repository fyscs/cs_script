...
let self = null;
let maxhp = 10000000.0;
let index = 1;
let ticking = false;
function Initialize() {
    // 直接查找名为 dd_hp 的实体
    const hpEntity = Entities.FindByName(null, 'dd_hp');
    if (!hpEntity || !hpEntity.IsValid()) {
        printl('[diddledickhealth] Initialize: dd_hp not found or invalid!');
        return;
    }
    self = hpEntity;
    maxhp = self.GetHealth();
    if (!ticking) {
        ticking = true;
        Tick();
    }
...
function Tick() {
    if (!self || !self.IsValid() || self.GetHealth() <= 0) {
        ticking = false;
        return;
    }
    const ratio = self.GetHealth() / maxhp;
    if (index == 1 && ratio <= 0.75) {
        index++;
        EntFire('dd_case_2', 'FireUser1', '', 0.0, null);
    }
    else if (index == 2 && ratio <= 0.5) {
        index++;
        EntFire('dd_case_3', 'FireUser1', '', 0.0, null);
    }
    else if (index == 3 && ratio <= 0.25) {
        index++;
        EntFire('dd_case_4', 'FireUser1', '', 0.0, null);
    }
    scheduleScript('ze_diddle/diddledickhealth', () => Tick(), 0.01);
}
function SpawnZombie() {
    const zombies = [];
    let player = null;
    while (null != (player = Entities.FindByClassname(player, 'player'))) {
        if (player.IsValid() && player.GetHealth() > 0 && getTeam(player) == 2) {
            zombies.push(player);
        }
    }
    if (zombies.length === 0)
        return;
    const zombie = zombies[RandomInt(0, zombies.length - 1)];
    if (!zombie || !zombie.IsValid())
        return;
    setOrigin(zombie, Vector(-14592, -14048, 2688));
    EntFireByHandle(zombie, 'SetHealth', '666', 0.0, null, null);
    setVelocity(zombie, Vector());
}
const EXTERNAL_INPUT_ALIASES = [
    input('Initialize', 'Initialize()', () => Initialize(), 'vmf+stripper'),
    input('SpawnZombie', 'SpawnZombie()', () => SpawnZombie(), 'vmf+stripper'),
];
registerInputAliases('ze_diddle/diddledickhealth', EXTERNAL_INPUT_ALIASES);
installScheduler();
