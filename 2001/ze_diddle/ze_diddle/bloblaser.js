...
//===================================\\
// Blob Laser script by Luffaren (STEAM_0:1:22521282)
// CS2 Port - ignore player self, fixed eye offset
// ===================================\\
function input(name, _legacyExpression, handler, source = "vmf", notes = "") {
    return { name, handler, source, notes };
}
// ---------- 脚本状态 ----------
const SCRIPT_PREFIX = 'ze_diddle/bloblaser';
const SCRIPT = "Map_BlobLaser";
const BLOB_CENTER = { x: 11320, y: 2608, z: 2616 };
const EYE_OFFSET = 48;
// ---------- 伤害相关 ----------
let damageLoopActive = false;
const DAMAGE_INTERVAL = 0.1; // 0.1秒检测一次
const DAMAGE_PER_HIT = 2; // 每次2点，即20点/秒
// ---------- 通用内部函数 ----------
function executeLaser(targetName, laserName, relayName, temName, onDelay) {
    const scriptEnt = Instance.FindEntityByName(SCRIPT);
    const players = Instance.FindEntitiesByClass("player");
    const plist = [];
    for (const player of players) {
        if (player && player.IsValid() && player.GetTeamNumber() === 3 && player.GetHealth() > 0) {
            const ppos = player.GetAbsOrigin();
            const traceStart = { x: ppos.x, y: ppos.y, z: ppos.z + EYE_OFFSET };
            const ignoreList = [scriptEnt, player].filter(e => e && e.IsValid());
            const trace = Instance.TraceLine({
                start: traceStart,
                end: BLOB_CENTER,
                ignoreEntity: ignoreList.length ? ignoreList : undefined
            });
    ...
    let selected = null;
    if (plist.length > 0) {
        const target = plist.length === 1 ? plist[0] : plist[Math.floor(Math.random() * plist.length)];
        if (target && target.IsValid() && target.GetHealth() > 0) {
            selected = target;
            ////printl(`[BlobLaser] Laser ${laserName} targeting player: ${target.GetEntityName()}`);
            const poss = target.GetAbsOrigin();
            const pos = `${poss.x} ${poss.y} ${poss.z + EYE_OFFSET}`;
            const targetEnt = Instance.FindEntityByName(targetName);
            if (targetEnt && targetEnt.IsValid()) {
                Instance.EntFireAtName({ name: targetName, input: "KeyValues", value: "origin " + pos });
            }
            const laserEnt = Instance.FindEntityByName(laserName);
            if (laserEnt && laserEnt.IsValid()) {
                Instance.EntFireAtName({ name: laserName, input: "Start", delay: 0.10 });
                Instance.EntFireAtName({ name: laserName, input: "Stop", delay: onDelay });
            }
            const temEnt = Instance.FindEntityByName(temName);
            if (temEnt && temEnt.IsValid()) {
                Instance.EntFireAtName({ name: temName, input: "KeyValues", value: "origin " + pos });
                Instance.EntFireAtName({ name: temName, input: "ForceSpawn", delay: 0.10 });
            }
            Instance.EntFireAtName({ name: relayName, input: "Disable" });
            Instance.EntFireAtName({ name: relayName, input: "Enable", delay: onDelay });
        }
        else {
            Instance.EntFireAtName({ name: relayName, input: "Enable" });
        }
    }
    else {
        Instance.EntFireAtName({ name: relayName, input: "Enable" });
...
function startDamageLoop() {
    ...
    //printl("[BlobLaser] Damage loop started");
    damageLoopTick();
}
function damageLoopTick() {
    if (!damageLoopActive)
        return;
    const start = BLOB_CENTER;
    const targetEnt = Instance.FindEntityByName("blobb_laser_target1");
    if (!targetEnt || !targetEnt.IsValid()) {
        //printl("[BlobLaser] Target entity missing, stopping damage loop");
        stopDamageLoop();
        return;
    }
    const end = targetEnt.GetAbsOrigin();
    const scriptEnt = Instance.FindEntityByName(SCRIPT);
    const trace = Instance.TraceLine({
        start,
        end,
        ignoreEntity: scriptEnt && scriptEnt.IsValid() ? scriptEnt : undefined,
        ignorePlayers: false
    });
    if (trace.didHit && trace.hitEntity && trace.hitEntity.IsValid()) {
        const hit = trace.hitEntity;
        if (getClassname(hit) === "player" && hit.GetTeamNumber() === 3 && hit.GetHealth() > 0) {
            const newHealth = hit.GetHealth() - DAMAGE_PER_HIT;
            EntFireByHandle(hit, "SetHealth", String(newHealth), 0.0, null, null);
            //printl(`[BlobLaser] Damaged player, new health: ${newHealth}`);
        }
    }
    // 继续循环
    scheduleScript(SCRIPT_PREFIX, () => damageLoopTick(), DAMAGE_INTERVAL);
}
function stopDamageLoop() {
    damageLoopActive = false;
    //printl("[BlobLaser] Damage loop stopped");
}
// ---------- 处理器函数 ----------
function handleStartLaser() {
    //printl("[BlobLaser] StartLaser called");
    executeLaser("blobb_laser_target", "blobb_laser", "blobb_laser_relay", "blobblaser_tem", 3.10);
    // 不添加伤害
}
function handleStartLaser1() {
    //printl("[BlobLaser] StartLaser1 called");
    const target = executeLaser("blobb_laser_target1", "blobb_laser1", "blobb_laser_relay1", "blobblaser_tem1", 6.10);
    if (target && target.IsValid()) {
        //printl("[BlobLaser] Starting damage loop due to valid target");
        startDamageLoop();
        // 6.1秒后停止伤害
        scheduleScript(SCRIPT_PREFIX, () => stopDamageLoop(), 6.10);
    }
}
// ---------- 外部输入别名列表 ----------
const EXTERNAL_INPUT_ALIASES = [
    input("StartLaser", "StartLaser()", handleStartLaser, "vmf", "Small laser targeting visible CT"),
    input("StartLaser1", "StartLaser1()", handleStartLaser1, "vmf", "Big laser targeting visible CT with damage"),
];
// ---------- 注册 ----------
