...
//===================================\\
// Didldedick Boss HP script by Luffaren (STEAM_0:1:22521282)
// CS2 Port - manager.ts style
// ===================================\\
function input(name, _legacyExpression, handler, source = "vmf", notes = "") {
    return { name, handler, source, notes };
}
// ---------- 核心更新函数 ----------
function UpdateBossHP(bossName, textName) {
    const boss = Instance.FindEntityByName(bossName);
    const worldText = Instance.FindEntityByName(textName);
    ...
    const health = Math.floor(boss.GetHealth());
    Instance.EntFireAtTarget({
        target: worldText,
        input: "SetMessage",
        value: `BOSS HP: ${health}`
    });
}
// ---------- 处理器函数（每个BOSS一个） ----------
function handleBoss1() { UpdateBossHP("X69XTurtleBossHP1", "X69XTurtleBossHP1_Hint"); }
function handleBoss2() { UpdateBossHP("X69XTurtleBosshp2", "X69XTurtleBosshp2_Hint"); }
function handleBoss3() { UpdateBossHP("Ord_lvl_01_boss_break", "Ord_lvl_01_boss_break_Hint"); }
function handleBoss4() { UpdateBossHP("Ord_lvl_02_boss_break", "Ord_lvl_02_boss_break_Hint"); }
function handleBoss5() { UpdateBossHP("X69XOrd_main_mid_diglett_break", "X69XOrd_main_mid_diglett_break_Hint"); }
function handleBoss6() { UpdateBossHP("X69XOrd_main_large_diglett_break", "X69XOrd_main_large_diglett_break_Hint"); }
// 如需更多，继续添加 handleBoss6...
// ---------- 外部输入别名列表 ----------
const EXTERNAL_INPUT_ALIASES = [
    input("DisplayBossHP_1", "DisplayBossHP_1()", handleBoss1, "vmf", "Update HP of Boss 1"),
    input("DisplayBossHP_2", "DisplayBossHP_2()", handleBoss2, "vmf", "Update HP of Boss 2"),
    input("DisplayBossHP_3", "DisplayBossHP_3()", handleBoss3, "vmf", "Update HP of Boss 3"),
    input("DisplayBossHP_4", "DisplayBossHP_4()", handleBoss4, "vmf", "Update HP of Boss 4"),
    input("DisplayBossHP_5", "DisplayBossHP_5()", handleBoss5, "vmf", "Update HP of Boss 5"),
    input("DisplayBossHP_6", "DisplayBossHP_6()", handleBoss6, "vmf", "Update HP of Boss 6"),
];
// ---------- 注册函数 ----------
function registerInputAliases() {
    ...
    //Instance.Msg(`[BossHP] Registered ${EXTERNAL_INPUT_ALIASES.length} inputs\n`);
}
// 顶层执行注册
