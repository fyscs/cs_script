...
const SCRIPT_PREFIX = 'ze_diddle/diddlejihad';
const DELAY = 0.5; // 延迟秒数，可调整
/**
 * 检测当前按钮附近是否有 "NO_JIHAD" 实体
 * 若有则立即触发 FireUser3（取消），若无则启动延迟二次检测
 */
function CheckJihad(inputData) {
    const caller = inputData.caller;
    if (!isValidEntity(caller)) {
        //Instance.Msg(`[${SCRIPT_PREFIX}] CheckJihad: caller is invalid\n`)
        return;
    }
    const origin = getOrigin(caller);
    const noJihad = Entities.FindByNameWithin(null, 'NO_JIHAD', origin, 512);
    if (isValidEntity(noJihad)) {
        // 第一次检测就发现有 NO_JIHAD，立即禁止
        EntFireByHandle(caller, 'FireUser3', '', 0.0, null, null);
        return;
    }
    // 第一次检测通过，启动延迟二次检测
    //Instance.Msg(`[${SCRIPT_PREFIX}] First check passed, scheduling second check in ${DELAY}s\n`)
    scheduleScript(SCRIPT_PREFIX, () => delayedCheck(caller), DELAY);
}
//延迟二次检测：再次检查 NO_JIHAD，若没有则触发爆炸（FireUser4），若有则取消（FireUser3）
function delayedCheck(caller) {
    if (!isValidEntity(caller)) {
        //Instance.Msg(`[${SCRIPT_PREFIX}] delayedCheck: caller invalid\n`)
        return;
    }
    const origin = getOrigin(caller);
    const noJihad = Entities.FindByNameWithin(null, 'NO_JIHAD', origin, 512);
    if (isValidEntity(noJihad)) {
        // 二次检测发现有 NO_JIHAD，取消爆炸
        //Instance.Msg(`[${SCRIPT_PREFIX}] Second check found NO_JIHAD, cancelling explosion\n`)
        EntFireByHandle(caller, 'FireUser3', '', 0.0, null, null);
    }
    else {
        // 二次检测通过，执行爆炸
        //Instance.Msg(`[${SCRIPT_PREFIX}] Second check passed, triggering explosion\n`)
        EntFireByHandle(caller, 'FireUser4', '', 0.0, null, null);
    }
}
const EXTERNAL_INPUT_ALIASES = [
    input('CheckJihad', 'CheckJihad()', (inputData) => CheckJihad(inputData), 'vmf'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
installScheduler();
////Instance.Msg(`[${SCRIPT_PREFIX}] script loaded with ${EXTERNAL_INPUT_ALIASES.length} external input aliases\n`)
