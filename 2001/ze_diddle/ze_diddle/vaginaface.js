...
let self = null;
const SCRIPT_PREFIX = 'ze_diddle/vaginaface';
let target = null;
let speed = 0.0;
...
function buildIgnoreList() {
    ...
    let other = null;
    while (null != (other = Entities.FindByName(other, 'i_vaginaface_model*'))) {
        if (other !== model && isValidEntity(other)) {
            list.push(other);
        }
    }
    return list;
...
function Start() {
    ...
    SetHealth();
    EntFire('Map_Manager', 'RunScriptInput', 'AddVagina', 0.0, self);
}
function CheckDiddleCannonProjectile() {
    let p = null;
    while (null !=
        (p = Entities.FindByNameWithin(p, 'projectile_hurt*', getOrigin(self), 100))) {
        EntFireByHandle(p, 'FireUser1', '', 0.0, null, null);
        EntFireByHandle(hp, 'RemoveHealth', '400', 0.0, null, null);
    }
}
function Tick() {
    if (!ticking)
        return;
    if (!dead && isValidEntity(target) && target.GetHealth() > 0) {
        retarget -= 0.02;
        ...
        if (retarget <= 0.0 || TraceLine(t1, t2, ignoreList, true) < 1.0) {
            ...
            const dir = Vector(t2.x - t1.x, t2.y - t1.y, t2.z - t1.z);
            const length = dir.length;
            const normal = length > 0.0 ? VectorScale(dir, 1.0 / length) : Vector();
            const newpos = VectorAdd(t1, VectorScale(normal, speed));
        ...
        if (!deadbig) {
            ...
            if (deadscale <= 0.0) {
                deadscale = 0.01;
                EntFireByHandle(model, 'Disable', '', 0.0, null, null);
            }
        }
...
function Die() {
    dead = true;
    EntFireByHandle(s_die, 'StartSound', '', 0.0, null, null);
    SetHurtAnimation();
}
function Hurt() {
    speed = 0.0;
    SetHurtAnimation();
    if (!hurtAnimSoundCooldown) {
        hurtAnimSoundCooldown = true;
        const rand = GetRandomValue(2);
        if (rand == 0)
            EntFireByHandle(s_hurt1, 'StartSound', '', 0.0, null, null);
        if (rand == 1)
            EntFireByHandle(s_hurt2, 'StartSound', '', 0.0, null, null);
        if (rand == 2)
            EntFireByHandle(s_hurt3, 'StartSound', '', 0.0, null, null);
        scheduleInternalScript(() => {
            hurtAnimSoundCooldown = false;
        }, 2.0, null, null);
    }
}
function HitPlayer() {
    EntFireByHandle(s_hit, 'StartSound', '', 0.0, null, null);
    speed = 0.0;
}
function SearchTarget() {
    ...
    while (null != (p = Entities.FindByClassname(p, 'player'))) {
        if (getTeam(p) == 3) {
            const ppos = p.GetEyePosition();
            if (TraceLine(getOrigin(self), ppos, ignoreList, true) == 1.0) {
                candidates.push(p);
            }
        }
    }
    ...
    if (hp != null && hp.IsValid()) {
        ...
        for (const controller of all) {
            ...
            if (!player || player.GetTeamNumber() != 3 || player.GetHealth() <= 0)
                continue;
            hpadd += foreachPlayerHpAdd;
        }
        EntFireByHandle(hp, 'SetHealth', String(baseHp + hpadd), 0.0, null, null);
    }
}
function GetRandomValue(max) {
    return RandomInt(0, max);
}
function SetHurtAnimation() {
    const rand = GetRandomValue(2);
    if (rand == 0)
        EntFireByHandle(model, 'SetAnimationNotLooping', 'hurt_1', 0.0, null, null);
    if (rand == 1)
        EntFireByHandle(model, 'SetAnimationNotLooping', 'hurt_2', 0.0, null, null);
    if (rand == 2)
        EntFireByHandle(model, 'SetAnimationNotLooping', 'hurt_3', 0.0, null, null);
}
function FaceDirection(direction) {
    const yaw = (Math.atan2(direction.y, direction.x) * 180.0) / Math.PI;
    const horizontal = sqrt(direction.x * direction.x + direction.y * direction.y);
...
const EXTERNAL_INPUT_ALIASES = [
    ...
    input('Hurt', 'Hurt()', () => Hurt(), 'vmf'),
    input('Die', 'Die()', () => Die(), 'vmf'),
    input('HitPlayer', 'HitPlayer()', () => HitPlayer(), 'vmf'),
    input('ExfaceHP', 'OverrideVaginaFaceHP()', () => ExfaceHP(), 'stripper'),
];
registerInputAliases(SCRIPT_PREFIX, EXTERNAL_INPUT_ALIASES);
installScheduler();
Instance.Msg('[ze_diddle] vaginaface script loaded with ' +
