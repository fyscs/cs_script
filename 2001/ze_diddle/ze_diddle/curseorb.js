...
function Tick() {
    if (!ticking)
        return;
    const orb = requireSelf(self, 'curseorb');
    const forward = getForwardVector(orb);
    setOrigin(orb, VectorAdd(getOrigin(orb), VectorScale(forward, SPEED)));
    if (TraceLine(getOrigin(orb), VectorAdd(getOrigin(orb), VectorScale(forward, 20)), orb, true) < 1.0) {
        EntFireByHandle(orb, 'FireUser1', '', 0.0, null, null);
    }
    scheduleScript('ze_diddle/curseorb', () => Tick(), 0.01);
}
const EXTERNAL_INPUT_ALIASES = [
    input('SetSelf', 'SetSelf()', (inputData) => SetSelf(inputData), 'new'),
    input('Init', 'Init()', () => Init(), 'vmf'),
];
registerInputAliases('ze_diddle/curseorb', EXTERNAL_INPUT_ALIASES);
installScheduler();
