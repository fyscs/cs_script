...
function forceSpawnTemplate(templateName, position = null, rotation = null) {
    ...
    if (classname !== 'point_template') {
        throw new Error(`[ze_diddle] '${templateName}' must be point_template, got '${classname}'`);
    }
    const spawned = template.ForceSpawn(position == null ? undefined : vec(position), rotation == null ? undefined : angles(rotation));
    if (spawned == null) {
        throw new Error(`[ze_diddle] ForceSpawn failed for '${templateName}'`);
    }
    const spawnedEntities = spawned.filter(isNotNull);
    if (spawnedEntities.length <= 0) {
        throw new Error(`[ze_diddle] ForceSpawn returned no entities for '${templateName}'`);
    }
...
//===================================\\
// Omaha Mortar script by Luffaren (STEAM_0:1:22521282)
// CS2 Port - manager.ts style, fixed inputs, SetNextThink driven
// ===================================\\
...
let minrate = 0.00;
let maxrate = 0.00;
let mortarOrigin = { x: 0, y: 0, z: 0 };
let isActive = false;
let nextFireTime = 0;
// ---------- 核心函数 ----------
function SpawnBarrage(_amount, _minrate, _maxrate, caller) {
    // 重置状态
    amount = _amount;
    minrate = _minrate;
    maxrate = _maxrate;
    if (caller && caller.IsValid()) {
        mortarOrigin = caller.GetAbsOrigin();
    }
    else {
        const center = Instance.FindEntityByName("mortar_center");
        if (center && center.IsValid()) {
            mortarOrigin = center.GetAbsOrigin();
        }
        else {
            mortarOrigin = { x: -12503.8, y: -2848, z: 13844 };
        }
    }
    // 如果已经激活，先停止
    if (isActive) {
        isActive = false;
        Instance.SetThink(() => { }); // 清空 Think
    }
    // 如果有炮弹要发射，启动循环
    if (amount > 0) {
        isActive = true;
        ...
        // 如果还有剩余，安排下一次
        if (amount > 0) {
            const rand = Math.random() * (maxrate - minrate) + minrate;
            nextFireTime = Instance.GetGameTime() + rand;
            Instance.SetThink(onThink);
            Instance.SetNextThink(nextFireTime);
        }
        else {
            isActive = false;
        }
...
function SpawnMortar() {
    if (amount <= 0)
        return;
    const x = Math.floor(mortarOrigin.x - 1312 + Math.random() * 2624);
    const y = Math.floor(mortarOrigin.y - 1312 + Math.random() * 2624);
    const z = mortarOrigin.z;
    const pos = { x, y, z };
    const zeroAngles = { pitch: 0, yaw: 0, roll: 0 };
    try {
        forceSpawnTemplate("s_mortar", pos, zeroAngles);
    }
    catch (e) {
        // 静默失败
    }
...
// ---------- 5 个处理器 ----------
function handleSpawnBarrage1(context) {
    SpawnBarrage(50, 0.5, 2.0, context.caller);
}
function handleSpawnBarrage2(context) {
    SpawnBarrage(30, 0.2, 1.5, context.caller);
}
function handleSpawnBarrage3(context) {
    SpawnBarrage(20, 1.0, 3.0, context.caller);
}
function handleSpawnBarrage4(context) {
    SpawnBarrage(50, 1.0, 3.0, context.caller);
}
function handleSpawnBarrage5(context) {
    SpawnBarrage(100, 0.2, 2.0, context.caller);
}
// ---------- 外部输入别名列表 ----------
const EXTERNAL_INPUT_ALIASES = [
    input("SpawnBarrage_1", "SpawnBarrage_1()", handleSpawnBarrage1, "vmf", "Barrage: 50 rounds, 0.5-2.0s interval"),
    input("SpawnBarrage_2", "SpawnBarrage_2()", handleSpawnBarrage2, "vmf", "Barrage: 30 rounds, 0.2-1.5s interval"),
    input("SpawnBarrage_3", "SpawnBarrage_3()", handleSpawnBarrage3, "vmf", "Barrage: 20 rounds, 1.0-3.0s interval"),
    input("SpawnBarrage_4", "SpawnBarrage_4()", handleSpawnBarrage4, "vmf", "Barrage: 50 rounds, 1.0-3.0s interval"),
    input("SpawnBarrage_5", "SpawnBarrage_5()", handleSpawnBarrage5, "vmf", "Barrage: 100 rounds, 0.2-2.0s interval"),
];
// ---------- 注册函数 ----------
