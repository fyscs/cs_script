...
function SpawnShrekHeads() {
    for (let index = 0; index <= 40; index++) {
        ...
        scheduleScript('ze_diddle/shrekheadpath', () => SpawnDoodler(), delay);
    }
    EntFire('X69Xluff_npc_phys2gg*', 'FireUser1', '', 25.0, null);
    EntFire('X69Xluff_npc_killme*', 'Enable', '', 25.0, null);
    EntFire('X69Xluff_npc_killme*', 'Enable', '', 26.0, null);
}
function SpawnDoodler() {
    forceSpawnTemplate('s_shrekface', Vector(RandomInt(-1e3, 5500), RandomInt(-13296, -12304), 13320));
}
const EXTERNAL_INPUT_ALIASES = [
    input('SpawnShrekHeads', 'SpawnShrekHeads()', () => SpawnShrekHeads(), 'vmf'),
];
registerInputAliases('ze_diddle/shrekheadpath', EXTERNAL_INPUT_ALIASES);
installScheduler();
