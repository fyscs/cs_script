...
function RenderCoinCount() {
    const item = requireSelf(self, 'diddlecannonammo');
    if (target != null && target.IsValid()) {
        EntFireByHandle(text, 'SetMessage', `AMMO: ${ammo}`, 0.0, target, target);
        //EntFireByHandle(text, 'Display', '', 0.02, target, target)
        if (getDistance(target, item) > 200)
            target = null;
    }
    scheduleScript('ze_diddle/diddlecannonammo', () => RenderCoinCount(), 0.2);
}
function UseItem() {
    ammo--;
    if (ammo <= 0)
        EntFireByHandle(requireSelf(self, 'diddlecannonammo'), 'FireUser2', '', 0.0, null, null);
}
function SetText(inputData) {
    text = requireEntity(inputData.caller, 'SetText caller');
}
function PickUp(inputData) {
    target = requireEntity(inputData.activator, 'PickUp activator');
}
function getDistance(a, b) {
    const aOrigin = getOrigin(a);
    const bOrigin = getOrigin(b);
    if (getClassname(a) == 'player')
        aOrigin.z += 48;
    else if (getClassname(b) == 'player')
        bOrigin.z += 48;
    return distance(aOrigin, bOrigin);
}
const EXTERNAL_INPUT_ALIASES = [
    input('SetSelf', 'SetSelf()', (inputData) => SetSelf(inputData), 'new'),
    input('RenderCoinCount', 'RenderCoinCount()', () => RenderCoinCount(), 'vmf'),
    input('UseItem', 'UseItem()', () => UseItem(), 'vmf'),
    input('SetText', 'SetText()', (inputData) => SetText(inputData), 'vmf'),
    input('PickUp', 'PickUp()', (inputData) => PickUp(inputData), 'vmf'),
];
registerInputAliases('ze_diddle/diddlecannonammo', EXTERNAL_INPUT_ALIASES);
installScheduler();
