...
function SetVelocity(inputData, x, y, z) {
    const activator = requireEntity(inputData.activator, 'SetVelocity activator');
    const velocity = getVelocity(activator);
    setVelocity(activator, Vector(x == -1 ? velocity.x : x, y == -1 ? velocity.y : y, z == -1 ? velocity.z : z));
}
const EXTERNAL_INPUT_ALIASES = [
    input('SetVelocity_0_0_0', 'SetVelocity(0,0,0)', (inputData) => SetVelocity(inputData, 0, 0, 0), 'vmf+stripper'),
    input('SetVelocity_M1_M1_400', 'SetVelocity(-1,-1,400)', (inputData) => SetVelocity(inputData, -1, -1, 400), 'vmf'),
    input('SetVelocity_0_200_300', 'self.SetVelocity(Vector(0,200,300))', (inputData) => SetVelocity(inputData, 0, 200, 300), 'stripper'),
];
registerInputAliases('ze_diddle/velocity', EXTERNAL_INPUT_ALIASES);
